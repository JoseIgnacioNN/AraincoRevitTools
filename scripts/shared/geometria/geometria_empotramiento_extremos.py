...
def _empotramiento_mm_desde_diametro(diam_nominal_mm, concrete_grade=None):
    # Preferir tablas G25/G35/G45 (mismo origen que traslape) cuando hay dosificacion.
    if concrete_grade is not None:
        try:
            from bimtools_rebar_hook_lengths import traslape_mm_from_nominal_diameter_mm

            try:
                d_try = float(diam_nominal_mm or DIAM_NOMINAL_RESPALDO_MM)
            except Exception:
                d_try = DIAM_NOMINAL_RESPALDO_MM
            L = traslape_mm_from_nominal_diameter_mm(d_try, concrete_grade)
            if L is not None and float(L) > 1e-6:
                try:
                    g = unicode(concrete_grade).strip().upper()
                except Exception:
                    g = str(concrete_grade)
                return float(L), u"{0} Ø{1:.0f} → emp. {2:.0f} mm".format(
                    g, float(d_try), float(L)
                )
        except Exception:
            pass
...
def _hook_mm_desde_diametro(diam_nominal_mm, concrete_grade=None):
...
def resolver_extremo_linea(
    ...
    if p_ext is None or dir_saliente_unit is None:
        return {
            ...
            u"modo": MODO_GANCHO,
            u"delta_mm": 0.0,
            u"hook_mm": _hook_mm_desde_diametro(d_mm, concrete_grade),
            u"emp_mm": 0.0,
            u"sonda_mm": mm_s,
    ...
    if choca:
        emp_mm, desc = _empotramiento_mm_desde_diametro(d_mm, concrete_grade)
        delta = mm_s + emp_mm
        try:
        ...
        if columnas_sonda:
            ...
            if p_ref is not None and ref_meta:
                try:
                    hook_mm = _hook_mm_desde_diametro(d_mm, concrete_grade)
