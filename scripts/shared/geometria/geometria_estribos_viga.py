...
def _array_length_ft_from_line_work(line_work):
    """Longitud útil de reparto (ft): eje recortado − 50 mm inicio − 50 mm fin."""
    try:
        ...
        return max(0.0, Lw - inset_ini - inset_fin)
    except Exception:
        return 0.0


def _normalize_zonas_override(zonas_override):
    """
    ``zonas_override``: lista de tuplas
    ``(Lz_ft, bar_type, spacing_mm, include_end_bars [, zone_kind])``.
    """
    if not zonas_override:
        return None
    out = []
    for row in zonas_override:
        ...
        sp = row[2]
        inc = bool(row[3])
        kind = row[4] if len(row) >= 5 else None
        out.append((Lz, bt, sp, inc, kind))
    return out or None


def _zonas_default_estribo(document, host, L_arr, bt_e, bt_c, sp_e, sp_c):
    """Regla legacy auto 2·h (sin override de dominio)."""
    ...
    if h_mm is not None and float(h_mm) > 0.0:
        ...
        if two_h_ft is not None and L_arr < float(two_h_ft) - 1e-9:
            zonas = [(L_arr, bt_c, sp_c, True, u"central")]
            ...
            if L_ext_tgt is not None and L_ext_tgt >= _MIN_EDGE_FT:
                ...
                if L_cent < min_len + _MIN_EDGE_FT:
                    zonas = [
                        (L_ext_each, bt_e, sp_e, True, u"extremo"),
                        (L_ext_each, bt_e, sp_e, True, u"extremo"),
                    ]
                else:
                    zonas = [
                        (L_ext_each, bt_e, sp_e, True, u"extremo"),
                        (L_cent, bt_c, sp_c, False, u"central"),
                        (L_ext_each, bt_e, sp_e, True, u"extremo"),
                    ]
    if not zonas:
        ...
        if L2 < min_len + _MIN_EDGE_FT:
            zonas = [(L_arr, bt_e, sp_e, True, u"extremo")]
        else:
            zonas = [
                (L1, bt_e, sp_e, True, u"extremo"),
                (L2, bt_c, sp_c, False, u"central"),
                (L3, bt_e, sp_e, True, u"extremo"),
            ]
    return zonas
...
def _crear_rebar_estribos_multizonas(
    ...
    ``zonas_override``: si se informa (dominio Armado vigas / UI), define el reparto
    en lugar de la regla 2·h local.
    """
    if (
        document is None
        or host is None
    ...
    try:
        ...
        sp_c = 200.0
    L_arr = _array_length_ft_from_line_work(line_work)
    ...
    if base_template is None:
        ...
        return 0
    override = _normalize_zonas_override(zonas_override)
    if override is not None:
        zonas = override
    else:
        zonas = _zonas_default_estribo(document, host, L_arr, bt_e, bt_c, sp_e, sp_c)
    cum = 0.0
    total_qty = 0
    zone_idx = 0
    try:
    ...
    for row in zonas:
        if len(row) >= 5:
            Lz, bt, sp_mm, include_end_bars, zone_kind_hint = row
        else:
            Lz, bt, sp_mm, include_end_bars = row[:4]
            zone_kind_hint = None
...
def _zonas_reparto_estribo_viga(
    ...
    Returns:
        ``(zonas, L_arr_ft)`` con zonas ``(Lz_ft, bar_type, spacing_mm, include_end_bars [, kind])``.
    """
    bt_e = bt_ext
    bt_c = bt_cent
    if bt_e is None:
    ...
    override = _normalize_zonas_override(zonas_override)
    if override is not None:
        return override, L_arr
    # _zonas_default incluye kind opcional; traba solo usa Lz/sp/include.
    zonas = _zonas_default_estribo(document, host, L_arr, bt_e, bt_c, sp_e, sp_c)
    return zonas, L_arr
