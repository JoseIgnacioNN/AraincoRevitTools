...
def _traslapo_longitudinal_mm_desde_bar_type(rebar_bar_type, concrete_grade=None):
    """
    Largo **total** de traslape (mm) entre tramos consecutivos.

    Si ``concrete_grade`` es G25/G35/G45, usa tablas BIMTools de dosificación.
    Si no, misma tabla e interpolación que ``lap_mm_para_bar_type`` en
    ...
    if rebar_bar_type is None:
        return 0.0, None
    # 1) Dosificación (G25/G35/G45) por Ø nominal del bar type.
    try:
        d_nom = float(_rebar_nominal_diameter_mm(rebar_bar_type))
    except Exception:
        d_nom = 0.0
    if concrete_grade is not None and d_nom > 0:
        try:
            from bimtools_rebar_hook_lengths import traslape_mm_from_nominal_diameter_mm

            L = traslape_mm_from_nominal_diameter_mm(d_nom, concrete_grade)
            if L is not None and float(L) > 1e-6:
                try:
                    g = unicode(concrete_grade).strip().upper()
                except Exception:
                    g = str(concrete_grade)
                return float(L), u"Traslape {0} Ø{1:.0f} → {2:.0f} mm".format(
                    g, d_nom, float(L)
                )
        except Exception:
            pass
...
def _split_line_by_distances_con_traslapos_empalme(line, cuts, lap_mm):
    ...
    ``lap_mm`` puede ser un escalar (mismo solape en todos los nudos) o una
    secuencia con un valor por corte (p. ej. traslape del mayor Ø adyacente).
    ...
    def _half_ft_for_cut(j):
        """Mitad de solape (pies) en el corte ``j`` (0-based)."""
        try:
            if isinstance(lap_mm, (list, tuple)):
                if j < 0 or j >= len(lap_mm):
                    lm = 0.0
                else:
                    lm = float(lap_mm[j] or 0.0)
            else:
                lm = float(lap_mm or 0.0)
        except Exception:
            lm = 0.0
        if lm <= 1e-9:
            return 0.0
        return 0.5 * _mm_to_ft(lm)
    ...
    for i in range(n_int):
        a, b = bounds[i], bounds[i + 1]
        if i > 0:
            h = _half_ft_for_cut(i - 1)
            if h > 1e-12:
                a = max(0.0, a - h)
        if i < n_int - 1 and i < n_cuts:
            h = _half_ft_for_cut(i)
            if h > 1e-12:
                b = min(L, b + h)
        if b - a < _MIN_LINE_LEN_FT:
            continue
