# Pegar TODO este bloque de una vez en la consola RPS (>>>).
# Revit 2025 | IronPython 3.4 — inyecta Armadura_Malla / Ubicacion / Nivel en barras de Area Reinforcement.
exec(r"""
import clr
...
from Autodesk.Revit.DB import (
    ...
    Floor, GeometryInstance, Options, Solid, StorageType, Transaction,
    UnitUtils, UnitTypeId,
)
from Autodesk.Revit.DB.Structure import (
    AreaReinforcement, MultiplanarOption, Rebar, RebarInSystem,
)
...
def _nivel_losa_como_string(floor):
    ...
    try:
        ...
        lid = None
    if lid is None:
        for bip_name in (u"INSTANCE_REFERENCE_LEVEL_PARAM", u"LEVEL_PARAM", u"SCHEDULE_LEVEL_PARAM"):
            try:
...
def _z_promedio_barra(barra):
    ...
    for include_all in (True, False):
        try:
            opt = MultiplanarOption.IncludeAllMultiplanarCurves if include_all else MultiplanarOption.IncludeOnlyPlanarCurves
...
def collect_all_area_reinforcements(document):
    return list(FilteredElementCollector(document).OfClass(AreaReinforcement).WhereElementIsNotElementType())
...
def inyectar_params_malla_en_proyecto(document):
    ...
    stats = {
        u"n_areas": len(areas), u"n_barras": 0, u"n_malla_ok": 0,
        u"n_ubicacion_ok": 0, u"n_nivel_ok": 0, u"n_sin_barras": 0,
        u"n_sin_nivel": 0, u"n_sin_ubicacion": 0, u"areas_detalle": [],
    }
    if not areas:
        return stats
    ...
    try:
        ...
        for ar in areas:
            ...
            stats[u"areas_detalle"].append({
                u"area_id": ar_id, u"n_barras": len(barras), u"nivel": nivel_valor,
                u"malla": n_m, u"ubicacion": n_u, u"nivel_ok": n_n,
            })
        t.Commit()
    except Exception:
        if t.HasStarted():
...
def _format_resumen(stats):
    ...
    if stats.get(u"n_sin_barras"):
        lines.append(u"Areas sin barras: {0}".format(stats[u"n_sin_barras"]))
    if stats.get(u"n_sin_nivel"):
        lines.append(u"Areas sin nivel de losa: {0}".format(stats[u"n_sin_nivel"]))
    if stats.get(u"n_sin_ubicacion"):
        lines.append(u"Barras sin ubicacion resoluble: {0}".format(stats[u"n_sin_ubicacion"]))
    for det in (stats.get(u"areas_detalle") or [])[:12]:
        lines.append(
            u"  - Area {0}: {1} barra(s), Nivel={2}, Malla={3}, Ubic={4}, NivelOK={5}".format(
                det.get(u"area_id"), det.get(u"n_barras"), det.get(u"nivel") or u"?",
                det.get(u"malla"), det.get(u"ubicacion"), det.get(u"nivel_ok"),
            )
        )
    n_det = len(stats.get(u"areas_detalle") or [])
    if n_det > 12:
        lines.append(u"  ... y {0} area(s) mas.".format(n_det - 12))
    return u"\n".join(lines)

areas = collect_all_area_reinforcements(doc)
if not areas:
    TaskDialog.Show(u"Arainco: Params malla Area Reinforcement", u"No hay Area Reinforcement en el proyecto.")
else:
