    ...
    if sys_ids is not None:
        ...
        for i in range(n):
            try:
                ...
                continue
    # RebarInSystem y Rebar libres (p. ej. tras HostStructuralRebar) comparten OST_Rebar.
    # No existe BuiltInCategory.OST_RebarInSystem en la API de Revit.
    try:
        dep = system_rein.GetDependentElements(
            ElementCategoryFilter(BuiltInCategory.OST_Rebar)
        )
    except Exception:
        dep = None
    if dep is None:
