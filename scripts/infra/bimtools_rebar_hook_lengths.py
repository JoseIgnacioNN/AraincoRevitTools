...
# Hormigón G25 — traslapes y empotramientos (mm) = tabla base BIMTools.
...
def normalize_concrete_grade(concrete_grade):
    """
  Normaliza dosificación de hormigón a ``G25`` / ``G35`` / ``G45``.

  Valores vacíos o no reconocidos → ``None`` (tabla base / legacy G25 en ganchos).
  """
    return _normalize_concrete_grade(concrete_grade)


def concrete_grade_from_text(text):
    """Extrae ``G25`` / ``G35`` / ``G45`` de un texto (p. ej. parámetro o material)."""
    if text is None:
        return None
    try:
        s = unicode(text).strip().upper()
    except Exception:
        try:
