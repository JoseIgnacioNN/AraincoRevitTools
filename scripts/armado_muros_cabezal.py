...
Por extremo: 1–25 capas empurradas en profundidad (longitudinal; punta libre);
encuentro L: hasta 25 en núcleo (+ post). 2–4 barras por capa repartidas en el
...
CABEZAL_MAX_CAPAS = 25  # punta libre / extremo libre
CABEZAL_MAX_CAPAS_ENCUENTRO = 25  # núcleo encuentro L (sin post)
CABEZAL_MIN_CAPAS = 1
# Capas post-encuentro (host, opt-in) + tope de lista layers.
CABEZAL_MAX_POST_CAPAS = 4
CABEZAL_MAX_CAPAS_TOTAL = 29  # 25 núcleo + 4 post
...
CABEZAL_LAYER_PITCH_MAX_MM = 2000.0
...
# Escenarios T1/T2/T3: 2..MAX (punta / núcleo / total con post).
CABEZAL_CONFINEMENT_SCENARIO_CAPAS = tuple(
    range(2, CABEZAL_MAX_CAPAS_TOTAL + 1)
)


def cabezal_layer_spacing_mm(ex_cfg=None, raw=None):
    """Paso c/c entre capas (mm). Default 150; clamp [MIN, MAX]."""
    v = None
    if raw is not None:
        try:
            ...
            v = None
    if v is None and ex_cfg and isinstance(ex_cfg, dict):
        try:
            cand = ex_cfg.get(u"layer_spacing_mm")
            if cand is not None:
                v = float(cand)
        except Exception:
            v = None
    if v is None or v != v or v <= 0:
        v = float(CABEZAL_LAYER_PITCH_MM)
    return max(
        float(CABEZAL_LAYER_PITCH_MIN_MM),
        min(float(CABEZAL_LAYER_PITCH_MAX_MM), float(v)),
    )


def cabezal_post_encuentro_activo(ex_cfg):
    """True si el extremo tiene capas post-encuentro activas (solo host / encuentro L)."""
    if not ex_cfg or not isinstance(ex_cfg, dict):
        return False
    if not cabezal_extremo_es_encuentro_l(ex_cfg):
        return False
    try:
        return bool(ex_cfg.get(u"post_encuentro_activo", False))
    except Exception:
        return False


def cabezal_n_capas_encuentro(ex_cfg):
    """Nº de capas en el núcleo del encuentro (sin post)."""
    if not ex_cfg or not isinstance(ex_cfg, dict):
        return CABEZAL_MIN_CAPAS
    try:
        n_tot = int(ex_cfg.get(u"n_capas", CABEZAL_MIN_CAPAS))
    except Exception:
...
def cabezal_n_capas_post(ex_cfg):
    """Nº de capas post-encuentro (0 si inactivo)."""
    if not cabezal_post_encuentro_activo(ex_cfg):
        return 0
    try:
        n_tot = int(ex_cfg.get(u"n_capas", 0))
    except Exception:
        n_tot = 0
    n_enc = cabezal_n_capas_encuentro(ex_cfg)
    return max(0, min(CABEZAL_MAX_POST_CAPAS, n_tot - n_enc))


def cabezal_n_capas_total(ex_cfg):
    """Capas activas totales (encuentro + post si activo)."""
    if not ex_cfg or not isinstance(ex_cfg, dict):
        return CABEZAL_MIN_CAPAS
    try:
        n_tot = int(ex_cfg.get(u"n_capas", CABEZAL_MIN_CAPAS))
...
def cabezal_confinement_perp_tie_layer_indices(n_capas, n_capas_encuentro=None):
    """
    Índices de capa (0-based) con traba ⊥ (Tipo 2/3 encuentro).

    Excluye solo extremos del tramo activo: 1ªC (0) y última capa (n-1).
    Con post-encuentro, la última es la última capa post; la última de
    encuentro sí recibe traba ⊥. ``n_capas_encuentro`` se ignora (compat).
    """
    try:
        n = int(n_capas)
    except Exception:
    ...
    if n <= 2:
        return []
    _ = n_capas_encuentro  # API compat; exclusión solo por extremos totales.
...
def cabezal_confinement_layout_spec_encuentro(
    n_capas, conf_type, n_capas_encuentro=None,
):
    """
    ...
    Tipo 2 (enc_fiber_perp): estribo + trabas ⊥ host
      — interiores [1 .. n-2] (excluye 1ªC y última capa; con post, última = post).
    Tipo 3 (enc_fiber_cross): igual Tipo 2 (+ cross-ties vía
    ``cabezal_confinement_cross_tie_bar_indices`` en pos. 2…n−1 de barras).
    """
    try:
        n = int(n_capas)
    except Exception:
    ...
    if (
        ...
        ties = cabezal_confinement_perp_tie_layer_indices(n, n_capas_encuentro)
        return stirrup, ties
    return [], []


def cabezal_confinement_layout_spec(n_capas, conf_type, n_capas_encuentro=None):
    """
    Índices de capa para estribo perimetral y trabas según escenario.

    Extremo libre (2..25 capas):
    ...
    ``n_capas_encuentro`` solo aplica a encuentro con capas post.
    """
    if cabezal_confinement_is_encuentro(conf_type):
        return cabezal_confinement_layout_spec_encuentro(
            n_capas, conf_type, n_capas_encuentro=n_capas_encuentro,
        )
    try:
        n = int(n_capas)
    except Exception:
...
def cabezal_stamp_confinement_type(ex_cfg, conf_type, doc=None, fallback_bar_type_id=None):
    ...
    post_on = cabezal_post_encuentro_activo(ex_cfg)
    ex_cfg[u"confinement"] = normalize_cabezal_confinement(
        merged, n_capas, encuentro=is_enc,
        n_capas_encuentro=(
            cabezal_n_capas_encuentro(ex_cfg) if post_on else None
        ),
    )
...
# Tope de etiquetas de confinamiento por txn en modo rápido (rollback acotado).
CABEZAL_TAGS_POR_LOTE_RAPIDO = 50
...
def default_cabezal_extremo_config():
    return {
        ...
        u"n_capas": 2,
        u"post_encuentro_activo": False,
        u"n_capas_encuentro": 2,
        u"bar_type_id": None,
...
def normalize_cabezal_confinement(
    raw, n_capas=None, encuentro=False, n_capas_encuentro=None,
):
    """Normaliza bloque ``confinement`` del extremo.

    Con ``encuentro=True`` migra tipos de extremo libre a ``enc_fiber*``.
    ``n_capas_encuentro`` solo aplica con capas post-encuentro activas.
    """
    try:
    ...
    stirrup_idx, tie_idx = cabezal_confinement_layout_spec(
        n, ctype, n_capas_encuentro=n_capas_encuentro,
    )
    if (
...
def cabezal_confinement_tie_layer_indices(
    conf, n_capas=None, n_capas_encuentro=None,
):
    ...
    if cabezal_confinement_scenario_applies(n):
        _, tie_idx = cabezal_confinement_layout_spec(
            n, ctype, n_capas_encuentro=n_capas_encuentro,
        )
        return list(tie_idx)
    _, tie_idx = cabezal_confinement_layout_spec(
...
def cabezal_build_confinement_jobs(
    ...
    n_capas = cabezal_effective_n_capas(ex_cfg)
    is_enc = cabezal_extremo_es_encuentro_l(ex_cfg)
    n_enc = cabezal_n_capas_encuentro(ex_cfg) if is_enc else None
    post_on = cabezal_post_encuentro_activo(ex_cfg)
    conf = normalize_cabezal_confinement(
        ex_cfg.get(u"confinement"), n_capas, encuentro=is_enc,
        n_capas_encuentro=n_enc if post_on else None,
    )
    ex_cfg[u"confinement"] = conf
    ...
    tie_idx = cabezal_confinement_tie_layer_indices(
        conf, n_capas, n_capas_encuentro=n_enc if post_on else None,
    )
    base = {
        u"wall": wall,
        u"wid": wid,
...
def _normalize_cabezal_extremo_layers(ex_cfg, n_tramos=1):
    ...
    if is_enc_l and post_on:
        ...
        if n_post < 1:
            n_post = 1
        n_capas = min(CABEZAL_MAX_CAPAS_TOTAL, n_enc + n_post)
        ...
        ex_cfg[u"post_encuentro_activo"] = False
    if not layers:
        n_capas = max(n_capas, CABEZAL_MIN_CAPAS if not is_enc_l else 2)
    while len(layers) < max_pad:
        if layers:
            src = layers[0]
        else:
    ...
    n_enc_conf = (
        cabezal_n_capas_encuentro(ex_cfg) if is_enc_l else None
    )
    ex_cfg[u"confinement"] = normalize_cabezal_confinement(
        ex_cfg.get(u"confinement"),
        n_capas,
        encuentro=is_enc_l,
        n_capas_encuentro=n_enc_conf if post_on else None,
    )
    ex_cfg[u"layer_spacing_mm"] = cabezal_layer_spacing_mm(ex_cfg)
    return layers
...
def cabezal_active_layers(ex_cfg):
    ...
    if not ex_cfg or not isinstance(ex_cfg, dict):
        return []
    layers = _cabezal_ex_cfg_layers(ex_cfg)
    n_capas = cabezal_n_capas_total(ex_cfg)
    if not layers:
        return []
    ...
    for ex in CABEZAL_EXTREMOS:
        ...
        if n_capas < min_capas or n_capas > max_capas:
            return False, u"Extremo «{0}»: n_capas={1} fuera de rango [{2},{3}].".format(
                ex, n_capas, min_capas, max_capas,
            )
        if cabezal_post_encuentro_activo(ex_cfg):
            n_enc = cabezal_n_capas_encuentro(ex_cfg)
            n_post = cabezal_n_capas_post(ex_cfg)
            ...
            if n_post < 1 or n_post > CABEZAL_MAX_POST_CAPAS:
                ...
                ).format(ex, n_post, CABEZAL_MAX_POST_CAPAS)
            if n_enc + n_post != n_capas:
                return False, (
                    u"Extremo «{0}»: n_capas ({1}) ≠ encuentro ({2}) + post ({3})."
                ).format(ex, n_capas, n_enc, n_post)
...
def _cabezal_layer_pitch_mm(thickness_mm, n_capas, cover_mm=None, requested_mm=None):
    ...
    n = max(1, int(n_capas))
    pitch = cabezal_layer_spacing_mm(raw=requested_mm)
    if n > 1:
...
def cabezal_enum_layer_index(layer_index, n_capas=None, extremo=None, ex_cfg=None):
    ...
    Por defecto coincide con ``layer_index`` de la UI. Encuentro L: misma regla
    en creación y preview (``layers[i] → xs[i]``, exterior → interior).
    """
    try:
        li = int(layer_index)
    except Exception:
...
def cabezal_encuentro_l_capa_slot_index(layer_index, n_capas=None, extremo=None):
    ...
    Convención estable: ``layer_index`` 0 (1ªC) → ``xs[0]`` (cara exterior del
    encuentro); índices crecientes hacia el interior del muro host armado.
    ``extremo`` no altera el mapeo (el preview WPF usa la misma regla).
    """
    _ = n_capas, extremo
    try:
        li = int(layer_index)
        ...
        if distrib_ft < 1e-12:
            _, _, distrib_ft, err_geom = _cabezal_capa_line_endpoints(
                ...
                layer_spacing_mm=float(
                    cabezal_layer_spacing_mm(ex_cfg),
                ),
                doc=doc, ex_cfg=ex_cfg,
            )
            if err_geom:
...
def _cabezal_stirrup_envelope_encuentro_fiber_ft(
    ...
    conf = normalize_cabezal_confinement(
        ...
        encuentro=True,
        n_capas_encuentro=(
            cabezal_n_capas_encuentro(ex_cfg)
            if cabezal_post_encuentro_activo(ex_cfg)
            else None
        ),
    )
    ...
    if zone is not None:
        ...
        if cabezal_post_encuentro_activo(ex_cfg):
            n_tot = cabezal_effective_n_capas(ex_cfg)
            layer_spacing_mm = float(
                cabezal_layer_spacing_mm(ex_cfg),
            )
            pad_mm = (
            ...
            for li in (0, max(0, n_tot - 1)):
                ...
                if conf_bt is None:
                    conf_bt = bar_type
                p_lo, _p_hi, _distrib, err_geom = _cabezal_capa_line_endpoints(
                ...
                if err_geom or p_lo is None:
                    continue
                lo_c, _tr = _cabezal_project_pt_extremo_axes_ft(geom, p_lo)
                bar_r_ft = _mm_to_internal(_bar_diameter_mm(bar_type) * 0.5)
                lo = float(lo_c)
                long_ext.extend([
                    lo - bar_r_ft - pad_ft,
            ...
            if long_ext:
                long_min = min(long_min, min(long_ext))
                long_max = max(long_max, max(long_ext))
...
def _cabezal_tie_across_layers_geometry_ft(
    ...
    Encuentro L (+ post): las capas núcleo se separan en el espesor del vecino,
    no solo en el longitudinal del host. Se usan posiciones reales XY de 1ªC y
    última capa (post) para no colapsar el tramo en la última capa de encuentro.
    """
    try:
        bi = int(bar_index)
    except Exception:
    ...
    # ── Encuentro L: tramo en XY real (1ªC → última / post) ─────────────────
    if cabezal_extremo_es_encuentro_l(ex_cfg):
        bar_world = []  # (XYZ @ z_plane, bar_type)
        for li in end_layer_indices:
            ...
            if err_geom or _p_lo is None:
                return None, None, None, None, err_geom or u"Tipo 3: sin eje de capa."
            try:
            ...
            # Eje maestro = barra 0; layout hacia -normal (igual CreateFromCurves).
            coords = _cabezal_layer_bar_trans_coords_ft(0.0, distrib_ft, nb_ly)
            if not coords or bi >= len(coords):
                return None, None, None, None, u"Tipo 3: sin cota transversal."
            try:
                base = XYZ(float(_p_lo.X), float(_p_lo.Y), z_plane)
                off = float(coords[bi]) - float(coords[0])
                pt = base.Add(n_muro.Multiply(off))
            except Exception as ex_pt:
                return None, None, None, None, u"Tipo 3: punto barra: {0}".format(str(ex_pt))
            bar_world.append((pt, bar_type))

        if len(bar_world) < 2:
            return None, None, None, None, u"Tipo 3: faltan extremos de traba."

        p_bar0 = bar_world[0][0]  # 1ªC
        p_bar1 = bar_world[1][0]  # última (post si aplica)
        try:
            d_vec = XYZ(
                float(p_bar1.X) - float(p_bar0.X),
                float(p_bar1.Y) - float(p_bar0.Y),
                0.0,
            )
            d_len = float(d_vec.GetLength())
        except Exception:
            d_len = 0.0
        if d_len < 1e-9:
            return None, None, None, None, u"Tipo 3: 1ªC y última capa colapsadas en planta."
        d_hat = d_vec.Normalize()

        tie_r_ft = _mm_to_internal(tie_diam_mm * 0.5)
        bar_r_ft = 0.0
        for _pt, bt in bar_world:
            try:
                bar_r_ft = max(bar_r_ft, _mm_to_internal(_bar_diameter_mm(bt) * 0.5))
            except Exception:
                pass
        if bar_r_ft < 1e-9:
            bar_r_ft = _mm_to_internal(_bar_diameter_mm(bar_world[0][1]) * 0.5)
        tie_offset_ft = float(bar_r_ft) + float(tie_r_ft)
        embrace_ft = float(bar_r_ft) + float(tie_r_ft) + float(tie_r_ft)

        # Offset de pata: -normal_muro en planta (misma convención que punta).
        try:
            n_flat = XYZ(float(n_muro.X), float(n_muro.Y), 0.0)
            if float(n_flat.GetLength()) > 1e-9:
                n_flat = n_flat.Normalize()
                if abs(float(n_flat.DotProduct(d_hat))) > 0.95:
                    n_flat = XYZ(-float(d_hat.Y), float(d_hat.X), 0.0)
                    if float(n_flat.GetLength()) > 1e-9:
                        n_flat = n_flat.Normalize()
            else:
                n_flat = XYZ(-float(d_hat.Y), float(d_hat.X), 0.0)
        ...
        def _shift(pt, along, across):
            out = pt
            try:
                if abs(float(along)) > 1e-12:
                    out = out.Add(d_hat.Multiply(float(along)))
                if abs(float(across)) > 1e-12:
                    out = out.Add(n_flat.Multiply(float(across)))
            except Exception:
                pass
            return XYZ(float(out.X), float(out.Y), z_plane)

        p_on0 = _shift(p_bar0, 0.0, -tie_offset_ft)
        p_on1 = _shift(p_bar1, 0.0, -tie_offset_ft)
        p_start = _shift(p_bar0, -embrace_ft, -tie_offset_ft)
        p_end = _shift(p_bar1, embrace_ft, -tie_offset_ft)
        interior_pt = _shift(
            XYZ(
                0.5 * (float(p_bar0.X) + float(p_bar1.X)),
                0.5 * (float(p_bar0.Y) + float(p_bar1.Y)),
                z_plane,
            ),
            0.0,
            0.0,
        ...
        try:
            cl = List[Curve]()
            prev = p_start
            for p_on, p_bar in ((p_on0, p_bar0), (p_on1, p_bar1)):
                p_bar_z = XYZ(float(p_bar.X), float(p_bar.Y), z_plane)
                if prev.DistanceTo(p_on) > tol:
                    cl.Add(Line.CreateBound(prev, p_on))
                if p_on.DistanceTo(p_bar_z) > tol:
                    cl.Add(Line.CreateBound(p_on, p_bar_z))
                if p_bar_z.DistanceTo(p_on) > tol:
                    cl.Add(Line.CreateBound(p_bar_z, p_on))
                prev = p_on
    ...
    # ── Extremo libre (punta): capas separadas en longitudinal del host ─────
...
def _create_cabezal_confinement_stirrup(
    ...
    is_enc_l = cabezal_extremo_es_encuentro_l(ex_cfg)
    post_on = cabezal_post_encuentro_activo(ex_cfg)
    n_eff = cabezal_effective_n_capas(ex_cfg)
    conf = normalize_cabezal_confinement(
        ex_cfg.get(u"confinement"),
        n_eff,
        encuentro=is_enc_l,
...
def _cabezal_aplicar_etiquetado_confinamiento_animado(doc, view, res, uidoc):
    ...
    # Animación: lotes chicos. Modo rápido: techo mayor (menos commits/Regenerate).
    try:
        if MODO_EJECUCION_RAPIDA:
            batch_tags = min(int(batch_tags), int(CABEZAL_TAGS_POR_LOTE_RAPIDO))
        else:
...
def _cabezal_aplicar_etiquetado_longitudinal_animado(doc, view, res, uidoc):
    ...
    # Animación: lotes chicos. Modo rápido: techo (igual que confinamiento).
    try:
        if MODO_EJECUCION_RAPIDA:
            batch_tags = min(int(batch_tags), int(CABEZAL_TAGS_POR_LOTE_RAPIDO))
        else:
