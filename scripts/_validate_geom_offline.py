...
def test_ceil_mm_to_nearest_10():
    _assert(ceil_mm_to_nearest_10(2530.0) == 2530.0, u"exact")
    _assert(ceil_mm_to_nearest_10(2531.0) == 2540.0, u"ceil +1")
    _assert(ceil_mm_to_nearest_10(2530.1) == 2540.0, u"ceil frac")
    _assert(ceil_mm_to_nearest_10(0.0) == 0.0, u"zero")


def test_fabricated_lengths_ui():
    L = 6318.0
    lap = 860.0
    cuts = [2100.0, 4200.0]
    spans = build_spans_mm(L, cuts)
    fab = fabricated_lengths_mm(L, cuts, lap)
    _assert(len(fab) == 3, u"3 fab")
    for i, sp in enumerate(spans):
        ext = span_fabrication_extension_mm(i, len(spans), lap)
        _assert(
            abs(fab[i] - (sp[u"length_mm"] + ext)) < 1e-6,
            u"fab ext T{0}".format(i + 1),
        )
    ok, msg, cuts2 = set_span_fabricated_length_mm(
        L, cuts, 0, fab[0] + 200.0, lap, 100.0
    )
    _assert(ok, msg or u"edit fab")
    fab2 = fabricated_lengths_mm(L, cuts2, lap)
    _assert(abs(fab2[0] - (fab[0] + 200.0)) < 1e-6, u"fab moved")
...
def main():
    ...
    test_ceil_mm_to_nearest_10()
    test_fabricated_lengths_ui()
    test_endpoint_modes()
