...
def _crear_tags_rebar_malla_tras_commit_lote(
    ...
    for wall in lote:
        ...
        if not eids:
            try:
                eids = rebars_por_muro_id.get(int(wid))
            except Exception:
                eids = None
        if not eids:
            continue
...
def _aplicar_etiquetas_malla_todos(
    ...
    ``stamp_pre_etiqueta=True`` asegura V./H. tras split desacople (copias nuevas).
    """
    ...
    if vista_rebar_tag_ok and view_etiqueta is not None and rebars_por_muro_id:
        ...
        for i0 in range(0, len(walls_ord), batch):
            ...
            # Sin refresh intermedio: el handler refresca tras Commit del txn padre.
...
def aplicar_unobscured_armado_muros_en_vista(
    ...
    - Barras de **malla**: solo ``UnhideElements`` + ``SetUnobscuredInView(False)``.
      Nunca View Unobscured ON (ni sólido ON) en la vista activa.
    ...
    for eid in rebar_ids_on:
        k = _element_id_int(eid)
        if k is not None and k in ids_malla:
            continue
        try:
            el = doc.GetElement(eid)
            if el is not None:
    ...
    def _aplicar_visibilidad():
        ...
        try:
            ...
            pass
        if rebars_malla:
            n_vis_local = _unhide_rebars_malla_en_vista(view, rebars_malla_por_muro_id)
            # Malla: forzar Unobscured OFF (nunca ON) en la vista activa.
            if apply_rebar_unobscured_off_in_view is not None:
                n_off_local = apply_rebar_unobscured_off_in_view(
                    doc, rebars_malla, view,
                ) or 0
            else:
                for rb in rebars_malla:
                    try:
        ...
        if rebars_on:
            ...
            try:
                ...
                pass
            n_on_local = apply_rebar_unobscured_in_view(doc, rebars_on, view) or 0
        return n_on_local, n_off_local, n_vis_local
    ...
    n_vis = 0
    try:
        from armado_muros_txn import TxnScope, doc_is_modifiable

        # Si ya hay txn padre abierta (p. ej. Mallas en muros), no abrir SubTxn extra.
        if doc_is_modifiable(doc):
            n_on, n_off, n_vis = _aplicar_visibilidad()
        else:
...
# Flujo solo Mallas en muros (handler preview UI).
_MALLAS_PBAR_BASE = u"Arainco: Mallas en muros"
_MALLAS_PBAR_PHASES = (
    u"Mallas",
    u"Etiquetas",
    u"Etiquetas muro",
...
def _unificado_pbar_start(doc, n_phases):
    ...
    Respeta ``pyrevit_progress_bar_enabled`` (Revit 2024–2026).
    """
    try:
...
def _mallas_pbar_phase_base(phase_name):
    return u"{} — {}".format(_MALLAS_PBAR_BASE, phase_name)


def _mallas_pbar_start(doc, n_phases):
    """
    ProgressBar de fases del flujo solo Mallas en muros.

    Mismo gating que unificado (Revit 2024–2026).
    """
    try:
        if not pyrevit_progress_bar_enabled(doc):
            return None
    ...
    if n_phases is None or int(n_phases) < 1:
        return None
    title = _ml_pbar_phase_title(
        _mallas_pbar_phase_base(_MALLAS_PBAR_PHASES[0]),
        n_phases,
    )
    try:
        from pyrevit import forms as _pyrevit_forms
...
def _merge_embed_resumen(dest, src):
    ...
    for k in (
        ...
        u"n_pie_muro_extended",
        u"n_pie_muro_sin_cambios",
        u"n_pie_muro_retract",
        u"n_pie_muro_pata_l",
        u"n_pie_muro_pata_l_ext",
        u"n_pie_muro_pata_l_int",
        u"n_vertical_split_desacople",
        u"n_horiz_retract",
...
def _post_procesar_rebars_lote(
    ...
    Mutaciones en un ``batch_mutation_scope`` (1 SubTransaction/Transaction): los
    ``TxnScope`` por barra quedan no-op anidados. Exclusión + stamp al final.

    Regenerates: 1 al inicio (bbox tras Remove System bajo txn padre) + 1 tras
    mutaciones verticales/horizontales (antes de exclusión/stamp). Sin regen final.
    ...
    def _run_post():
        ...
        from armado_muros_txn import batch_mutation_scope

        with batch_mutation_scope(
            doc, u"Arainco: Armado muros lineales — post lote mutaciones",
        ):
            try:
...
def asignar_rebar_cover_a_muro(wall, cover_ext_int, cover_other):
    ...
    - Exterior / Interior → ``cover_ext_int`` (25 mm) — obligatorias
    - Other Faces → ``cover_other`` (0 mm) — best-effort (en R2025+ a
      veces es solo lectura / no aplica; no bloquea el armado)

    Retorna ``(True, None)`` si ext+int OK.
    Retorna ``(True, aviso)`` si ext+int OK pero other falló.
    Retorna ``(False, error)`` si falta ext o int.
    """
    ...
    faltan_criticas = []
    if not ok_ext:
        faltan_criticas.append(u"exterior")
    if not ok_int:
        faltan_criticas.append(u"interior")
    if faltan_criticas:
        return False, u"no se pudo asignar: " + u", ".join(faltan_criticas)

    # Ext/int OK; other opcional.
    return True, u"otras caras no asignadas (parámetro no disponible o solo lectura)"
...
def _resolver_tipos_rebar_cover(doc, errores=None):
    ...
    if cover_other is None:
        # Sin tipo 0 mm: seguir con ext/int; other se omite en asignación.
        msg = (
            u"Aviso: no hay Rebar Cover {:.0f} mm (otras caras); "
            u"se usará solo ext/int {:.0f} mm."
        ).format(REBAR_COVER_MM_OTRAS_CARAS, REBAR_COVER_MM_CARAS_EXT_INT)
        if errores is not None:
            errores.append(msg)
        cover_other = cover_ext_int
    ...
    if _rebar_es_malla_vertical_para_exclusion(
        ...
        try:
            ...
            if ajustar_inclusion_extremos_rebar_set_con_fallback is None:
                return False
            # Junta desacople: no apagar el extremo que no llega al muro
...
def _crear_un_ar_muro_con_curvas(
    doc,
    wall,
    wid,
    curve_list,
    ...
    """Create AR + malla (+ remove opcional) con ``curve_list`` ya armada."""
...
def _crear_armadura_un_muro_lineal(
    ...
    defer_remove=False,
    pending_area_reins=None,
    walls_seleccion=None,
):
    """
    Crea AR (+ opcional Remove System / exclusión) para un muro (Transaction abierta).

    Un solo Area Reinforcement por muro (horizontales continuas). El desacople
    apilado se resuelve en post partiendo solo sets verticales.
    ...
    ``walls_seleccion``: reservado para post (apilado); no parte el AR en creación.
    ...
    if not skip_cover_assign:
        ...
        if not ok_cover:
            errores.append(
                u"Muro {}: Rebar Cover — {} (se continúa la malla).".format(
                    wid, err_cover or u"error",
                ),
            )
    ...
    return _crear_un_ar_muro_con_curvas(
        doc,
        wall,
        wid,
        curve_list,
...
def crear_areas_malla_parametrizada(doc, walls, params_por_muro_id,
    ...
    Barra de progreso pyRevit: un paso por muro (Revit 2024 + IronPython 2).
    """
    resumen_ok = []
    errores = []
    cover_asignados = 0
    ...
    params_por_muro_id = _normalize_muro_id_dict(params_por_muro_id) or {}
    ...
    # Un paso por muro (con lote único en modo rápido la barra sigue avanzando).
    _pb_count = n_muros if n_muros else n_lotes
    _pb_title = u"{} (armado + post)".format(_ML_PBAR_BASE)
    pb_wall_idx = 0
    ...
    try:
        _pb_crea = _ml_pbar_start(
            _ml_pbar_phase_title(_pb_title, _pb_count),
            _pb_count,
            doc=doc,
        )
        ...
        for i0 in range(0, n_muros, batch):
            ...
            try:
                for wall in lote:
                    ...
                    try:
                        _crear_armadura_un_muro_lineal(
                            doc,
                            wall,
                            params_dict,
                            layer_active_dict,
            ...
            if lote_ok and lote_wids_con_rebars:
                ...
                if rebars_lote:
                    # No dejar que un fallo de post-proceso aborte el TG
                    # (rollback de mallas ya creadas y commitadas en el lote).
                    try:
                        embed_resumen = _merge_embed_resumen(
                            embed_resumen,
        ...
        # Si ya hay rebars en lotes commitados, asimilar el TG (no borrar mallas).
        if resumen_ok or rebars_por_muro_id:
            creacion_ok = True
    finally:
        _ml_pbar_exit(_pb_crea, _pbar_crea_open)
        if use_own_tg and tg_started and tg is not None:
            try:
                if creacion_ok or resumen_ok or rebars_por_muro_id:
                    tg.Assimilate()
                else:
                    tg.RollBack()
            except Exception:
...
def _crear_armado_muros_unificado_impl(
    ...
    try:
        ...
        # 4/6 Mallas (omitido si ningún muro tiene malla activa — p. ej. V3 unificado)
        _ml_pbar_step(
            pb, 3, n_phases, _unificado_pbar_phase_base(_UNIF_PBAR_PHASES[3]),
        )
        any_malla = False
        if malla_activo_por_muro_id is None:
            any_malla = bool(walls_ord)
        else:
            for w in walls_ord:
                if malla_activo_por_muro_id.get(_wall_id_int(w), True):
                    any_malla = True
                    break
        if any_malla:
            ...
            ok = []
            cover_n = int(
                (embed_resumen or {}).get(u"n_rebar_cover_inicio", 0) or 0
            )
        ...
        if any_malla and rebars_por_muro_id:
...
def _open_preview_ui(revit, uidoc, walls, mode):
    try:
        ...
        try:
            from armado_muros_run import ensure_armado_muros_modules_fresh
...
def run_mallas(revit):
    uidoc = revit.ActiveUIDocument
    if uidoc is None:
        TaskDialog.Show(u"Arainco: Mallas en muros — Error", u"No hay ventana Revit activa.")
        return
    if not _guard_vista_armado_muros(uidoc, revit):
        return
    walls = _pick_muros(uidoc)
    if walls is None:
        return
    if not walls and not getattr(_pick_muros, "_rechazo_paralelo", False):
        TaskDialog.Show(u"Arainco: Mallas en muros", u"No seleccionaste muros válidos.")
        return
    if not walls:
        return
...
def run_unificado(revit):
    ...
    filtro_token = None
    try:
        from filtro_armadura_eje import suspend_armadura_eje_filters_in_active_view

        filtro_token = suspend_armadura_eje_filters_in_active_view(uidoc)
    except Exception:
        filtro_token = None
    try:
        walls = _pick_muros(uidoc)
        ...
        try:
            from filtro_armadura_eje import restore_armadura_eje_filters

            restore_armadura_eje_filters(filtro_token)
        except Exception:
            pass
