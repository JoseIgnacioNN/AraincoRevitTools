...
class SheetExportManager(object):
    ...
    __slots__ = ("_doc", "_app", "_year", "_sanitize", "_pdf_opts", "_dwg_opts_cache")
    ...
    def __init__(self, doc, sanitize_file_base_fn=None):
        ...
        self._pdf_opts = None
        self._dwg_opts_cache = {}

    def _pdf_options(self):
        if self._pdf_opts is None:
            ...
            try:
                ...
                pass
            self._pdf_opts = opts
        return self._pdf_opts

    def _dwg_options(self, setup_name):
        key = _unicode_safe(setup_name, u"") if setup_name is not None else u""
        cached = self._dwg_opts_cache.get(key)
        if cached is not None:
            try:
                cached.MergedViews = True
            except Exception:
                pass
            return cached
        dwg_opts = None
        if key:
            try:
                st = ExportDWGSettings.FindByName(self._doc, key)
            except Exception:
                st = None
            if st is not None:
                try:
        ...
        try:
            ...
            pass
        self._dwg_opts_cache[key] = dwg_opts
        return dwg_opts
    ...
    def export_dwg(self, folder, sheet_id, custom_base, dwg_setup_name=None):
        ...
        try:
            dwg_opts = self._dwg_options(dwg_setup_name)
            ids = List[ElementId]()
            ids.Add(sheet_id)
