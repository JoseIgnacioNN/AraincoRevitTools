...
def _read_revision_slots(sheet):
    ...
    Tras dos slots consecutivos vacíos se detiene (secuencia típica R01…R0n).
    """
    blocks = []
    empty_streak = 0
    for n in range(1, MAX_REVISION_BLOCKS + 1):
...
try {
    ...
    # --- HOJA: LISTADO DE PLANOS (rango, no celda a celda) ---
    $rowList = @($rows)
    $n = $rowList.Count
    if ($n -gt 0) {
        $listArr = New-Object 'object[,]' $n, 5
        for ($i = 0; $i -lt $n; $i++) {
            $item = $rowList[$i]
            $listArr.SetValue([string]$item.sheet_number, $i, 0)
            $listArr.SetValue([string]$item.rev_display, $i, 1)
            $listArr.SetValue([string]$item.emision, $i, 2)
            $listArr.SetValue([string]$item.fecha, $i, 3)
            $listArr.SetValue([string]$item.sheet_name, $i, 4)
        }
        $r1 = $C.first_data_row_listado
        $r2 = $r1 + $n - 1
        $wsL.Range("A${r1}:E${r2}").Value2 = $listArr
    }

    # --- HOJA: HISTORIAL DE REVISIONES ---
    $truncated = $false
    if ($n -gt 0) {
        $colsH = 2 + $C.block_step * $C.max_revision_blocks
        $histArr = New-Object 'object[,]' $n, $colsH
        for ($i = 0; $i -lt $n; $i++) {
            $item = $rowList[$i]
            $histArr.SetValue([string]$item.sheet_number, $i, 0)
            $histArr.SetValue([string]$item.sheet_name, $i, 1)
            $blocks = @($item.blocks)
            ...
            for ($b = 0; $b -lt $blocks.Count; $b++) {
                $blk = $blocks[$b]
                $c0 = 2 + $C.block_step * $b
                $histArr.SetValue([string]$blk.revision,    $i, ($c0 + 0))
                $histArr.SetValue([string]$blk.descripcion, $i, ($c0 + 1))
                $histArr.SetValue([string]$blk.dibujo,      $i, ($c0 + 2))
                $histArr.SetValue([string]$blk.reviso,      $i, ($c0 + 3))
                $histArr.SetValue([string]$blk.aprobo,      $i, ($c0 + 4))
                $histArr.SetValue([string]$blk.fecha,       $i, ($c0 + 5))
            }
        }
        $r1 = $C.first_data_row_historial
        $r2 = $r1 + $n - 1
        $endHCol = Col-Letter $colsH
        $wsH.Range("A${r1}:${endHCol}${r2}").Value2 = $histArr
    }
    ...
    try:
        ...
        if not proc.WaitForExit(180000):
            try:
                ...
                pass
            raise IOError(
                u"PowerShell tardó más de 180 s. "
