...
from exportar_laminas.lib.sheet_export_manager import SheetExportManager  # noqa: E402
...
class PdfExportStrategy(object):
    ...
    def _manager(self, doc):
        if self._mgr is None or self._mgr.document is not doc:
            self._mgr = SheetExportManager(doc, sanitize_file_base_fn=self._sanitize_fn)
        return self._mgr
    ...
    def execute(self, doc, pdf_dir, element_id, file_base):
        ...
        """
        return bool(self._manager(doc).export_pdf(pdf_dir, element_id, file_base))
...
class DwgExportStrategy(object):
    ...
    def execute(self, doc, dwg_dir, element_id, file_base):
        ...
        """
        return bool(
            self._manager(doc).export_dwg(dwg_dir, element_id, file_base, self._setup)
        )
