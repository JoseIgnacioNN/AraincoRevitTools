...
from exportar_laminas.lib.exportar_laminas_pdf_dwg import build_sheet_catalog  # noqa: E402
...
class ExportarLaminasViewModel(object):
    ...
    def __init__(
        ...
        # Carga inicial de datos (un pase de collector + parámetros)
    ...
    def list_naming_source_options(self):
        cat = self._catalog
        fn = self._list_naming_opts

        def _wrapped(doc):
            try:
                return fn(doc, catalog=cat)
            except TypeError:
                return fn(doc)
    ...
    def _begin_table_batch(self):
        self._batching_table = True
        try:
            if self._table is not None:
                self._table.BeginLoadData()
        except Exception:
            pass

    def _end_table_batch(self):
        try:
            if self._table is not None:
                self._table.EndLoadData()
        except Exception:
            pass
        self._batching_table = False
        self._notify_estado_changed()
    ...
    def _reload_from_model(self):
        """Recarga láminas, FCH y receta de nombres con un único catálogo."""
        self._catalog = build_sheet_catalog(self._doc)
        try:
            self._table = self._build_sheets(self._doc, catalog=self._catalog)
        except TypeError:
            self._table = self._build_sheets(self._doc)
        try:
            self._fch_param_names = self._list_fch(self._doc, catalog=self._catalog)
        except TypeError:
            ...
            try:
                ...
                if do_dwg and n_sel > 0:
                    ...
                    for i in selected_indices:
                        if ps.is_cancelled():
                            cancelled = True
                            errores.append(u"Exportación DWG cancelada por el usuario.")
                            break
                ...
                # -- Fase PDF -------------------------------------------------
                if do_pdf and n_sel > 0 and not cancelled:
                    ps.begin_phase(ps.phase_title(_PBAR_PDF, n_sel), n_sel)
                    sn = [0]
                    for i in selected_indices:
                        if ps.is_cancelled():
                            cancelled = True
                            errores.append(u"Exportación PDF cancelada por el usuario.")
                            break
                        got = self._parse_row_job(i)
                        if got is None:
                ...
                # -- Fase Listado Excel ----------------------------------------
                if do_listado and n_sel > 0 and not cancelled:
                    if ps.is_cancelled():
                        cancelled = True
                        errores.append(u"Exportación cancelada por el usuario.")
                    else:
