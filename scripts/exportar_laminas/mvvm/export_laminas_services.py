...
from exportar_laminas.ui.export_laminas_instruction_dialog import (  # noqa: E402
    show_message_dialog,
...
class ProgressService(object):
    ...
    def begin_phase(self, title, count):
        ...
        if c < 1:
            return
        try:
            pb = self._forms.ProgressBar(title=title, cancellable=True)
            try:
    ...
    def is_cancelled(self):
        """True si el usuario pulsó Cancelar en la barra de progreso."""
        if not self._pb_active or self._pb is None:
            return False
        try:
            return bool(getattr(self._pb, u"cancelled", False))
        except Exception:
            return False
