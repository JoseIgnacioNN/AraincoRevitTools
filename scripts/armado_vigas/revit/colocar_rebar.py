...
def _guide_audit_label(guide):
    layer = int(guide.get(u"layer") or 1)
    if guide.get(u"es_suple_superior"):
        return u"Suple sup. capa {0}".format(layer)
    if guide.get(u"es_suple_inferior"):
        return u"Suple inf. capa {0}".format(layer)
    cara = guide.get(u"cara") or u"?"
    tramo_id = guide.get(u"tramo_id")
    if tramo_id is not None:
        return u"{0} capa {1} · T{2}".format(cara, layer, tramo_id)
    return u"{0} capa {1}".format(cara, layer)
...
def find_longitudinal_guides_over_limit(document, session, max_mm=MAX_BAR_MM):
    """
    Audita guías longitudinales (sup/inf, suple sup/inf) sin colocar Rebar.

    Returns:
        Lista de ``{label, length_mm}`` con segmentos que superan ``max_mm``.
    """
...
def colocar_armadura_longitudinal(document, session):
    ...
    Returns:
        ...
        para etiquetado por cara y parámetros de capa.
    ...
    for es_inf in (False, True):
        ...
        for run in build_bar_runs(sorted_beams, es_cara_inferior=es_inf):
            ...
            for guide in guides or []:
                ...
                es_suple_inf = bool(guide.get(u"es_suple_inferior"))
                es_suple_sup = bool(guide.get(u"es_suple_superior"))
                if es_suple_inf:
                ...
                if qty > 0:
                    ...
                    if rb is not None:
                        ...
                        if (
                            ...
                            and not es_suple_inf
                            and not es_suple_sup
                        ):
    ...
    return n_total, avisos, rebars_creados, {
        u"sup": rebars_sup,
        u"inf": rebars_inf,
        u"layer_by_id": rebar_layer_by_id,
    }, lap_jobs_all
