...
def _focus_existing_window(existing):
    """Restaura ventana oculta/minimizada y la trae al frente."""
    if existing is None:
        return
    try:
        ...
        pass
    try:
        if hasattr(existing, "_restore_after_colocar_ui"):
            existing._restore_after_colocar_ui()
    except Exception:
        pass
    try:
...
class ArmadoVigasWindow(object):
    ...
    def _set_colocar_ui_busy(self, busy):
        try:
            btn = self._win.FindName(u"BtnColocar") if self._win else None
            if btn is not None:
                btn.IsEnabled = not bool(busy)
        except Exception:
            pass

    def hide_for_colocar(self):
        """Oculta la ventana mientras Revit coloca armadura (evita «No responde»)."""
        self._set_colocar_ui_busy(True)
        try:
            if self._win is not None:
                self._win.Hide()
        except Exception:
    ...
    def hide_for_colocar_on_ui(self):
        """Oculta la ventana desde ExternalEvent (hilo Revit → UI)."""
        self._run_on_ui_sync(self.hide_for_colocar)

    def _run_on_ui_sync(self, action):
    ...
    def restore_after_colocar(self):
        """Restaura ventana y botón Colocar tras error o cancelación."""
        self._dispatch_ui(self._restore_after_colocar_ui)

    def _restore_after_colocar_ui(self):
        self._set_colocar_ui_busy(False)
        try:
    ...
    def _on_select_stirrup_zone(self, idx, role):
        ...
        if 0 <= idx < len(beams):
            if role == u"confin":
                ...
                ini = beam_suple_sup_side_enabled(beam, "start")
                fin = beam_suple_sup_side_enabled(beam, "end")
                self.set_status(
                    u"Suple sup. · {0} · {1} · Ini {2} · Fin {3} · ø{4} · n={5}.".format(
                        beam.get("id"),
                        u"Sí" if on else u"No",
                        u"Sí" if ini else u"No",
                        u"Sí" if fin else u"No",
