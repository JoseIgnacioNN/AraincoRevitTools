...
def _es_losa_cimentacion_por_tipo(doc, element):
    """``Floor`` cuyo ``FloorType.IsFoundationSlab`` (Foundation Slab)."""
...
def _es_fundacion_unida_para_preview(doc, element):
    """Zapata/muro de cimentación o Foundation Slab unidos al muro."""
...
def _ancho_bbox_elemento_mm(elem):
    """Mayor extensión horizontal del bbox en mm (para dibujo de zapata/losa)."""
    if elem is None:
        return None
    try:
        bb = elem.get_BoundingBox(None)
        ...
        try:
            return float(
                UnitUtils.ConvertFromInternalUnits(w_ft, UnitTypeId.Millimeters)
            )
        except Exception:
    ...
    def _run_vertical_lote():
        ...
        for wid0 in rebars_por_muro_id:
            ...
            if host0 is not None and _fundaciones_estructurales_unidas_muro(doc, host0):
                need_fund_geom = True
                break
        if need_fund_geom and regenerate_fund_geom:
            try:
                doc.Regenerate()
            except Exception:
                pass
