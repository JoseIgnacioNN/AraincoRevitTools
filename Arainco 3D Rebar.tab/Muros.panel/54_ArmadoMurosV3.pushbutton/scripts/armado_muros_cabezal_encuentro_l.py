...
def cabezal_encuentro_l_refresh_pitch_in_cfg(ex_cfg, host_wall, neighbor_wall):
    ...
    cab = _cab()
    try:
        if cab is not None and cab.cabezal_post_encuentro_activo(ex_cfg):
            n_capas = int(cab.cabezal_n_capas_encuentro(ex_cfg))
        else:
...
def cabezal_encuentro_l_detected_wall_inward_at_join(
    neighbor_wall, p_join, host_vector_longitudinal,
):
    """
    ``inward`` unitario del muro detectado para apilar capas desde la cara
    exterior del encuentro (no la cara hacia el interior en L).

    Si ``Wall.Orientation`` apunta hacia el fuste del host, se invierte el eje
    de espesor para anclar ``xs[0]`` en la fibra exterior del encuentro.
    """
    ...
    if host_vector_longitudinal is not None:
        try:
            ext_hat = n_det.Negate()
            if float(ext_hat.DotProduct(host_vector_longitudinal)) > 0.05:
                n_det = n_det.Negate()
        except Exception:
            pass
    return n_det
...
def cabezal_encuentro_l_capa_line_endpoints(
    ...
    Núcleo: capas equitativas en espesor detectado (``n_capas`` = core N).
    Índice 0 (1ªC) = ``xs[0]`` desde la cara exterior del encuentro; índices
    crecientes hacia el interior del host armado (sin invertir por extremo).
    Post-encuentro (``layer_index >= n_enc``): offset en el host a lo largo de
    ``vector_longitudinal`` cada ``CABEZAL_LAYER_PITCH_MM`` (150 mm) desde la
    última capa del núcleo; barras en espesor del muro seleccionado.
    """
    cab = _cab()
    if cab is None:
    ...
    n_enc = n_capas
    post_on = False
    if ex_cfg is not None and cab is not None:
        post_on = cab.cabezal_post_encuentro_activo(ex_cfg)
        if post_on:
            n_enc = cab.cabezal_n_capas_encuentro(ex_cfg)
    try:
        nc = max(ENC_L_MIN_CAPAS, int(n_enc or ENC_L_MIN_CAPAS))
    except Exception:
        nc = ENC_L_MIN_CAPAS

    pitch_mm = float(getattr(cab, u"CABEZAL_LAYER_PITCH_MM", 150.0) if cab else 150.0)
    normal_muro = geom_h[u"normal_muro"]
    espesor_ft = float(geom_h[u"espesor_ft"])
    vector_long = geom_h[u"vector_longitudinal"]
    z_bot, z_top = cab._wall_z_bounds_ft(wall)

    if post_on and li >= nc:
        post_i = li - nc
        last_core_li = max(0, nc - 1)
        p_core, _p_hi, _dist_core, err_core = cabezal_encuentro_l_capa_line_endpoints(
            doc, wall, extremo, last_core_li, bar_type, conf_bar_type,
            neighbor_wall, n_capas=nc, cover_mm=cover_mm, ex_cfg=None,
        )
        if err_core or p_core is None:
            return None, None, None, err_core or u"Sin última capa núcleo (post)."
        try:
            offset_ft = cab._mm_to_internal((post_i + 1) * pitch_mm)
            inicio = p_core.Add(vector_long.Multiply(offset_ft))
        except Exception as ex_off:
            return None, None, None, u"Offset post L: {0}".format(str(ex_off))
    ...
    slot_i = li
    if cab is not None:
        slot_i = cab.cabezal_encuentro_l_capa_slot_index(li, nc, extremo)
    ...
    n_det = cabezal_encuentro_l_detected_wall_inward_at_join(
        neighbor_wall, p_join, vector_long,
    )
    if n_det is None:
...
def cabezal_seccion_preview_layout_encuentro_l(
    ...
    pitch_post = float(
        pitch_post_mm
        if pitch_post_mm is not None
        else (getattr(cab, u"CABEZAL_LAYER_PITCH_MM", 150.0) if cab else 150.0)
    )
    post_on = bool(post_encuentro_activo)
    try:
        n_enc = int(n_capas_encuentro if n_capas_encuentro is not None else len(layers))
    except Exception:
        n_enc = len(layers)
    if not post_on:
        n_enc = max(1, len(layers))
    n_enc = max(1, min(len(layers), n_enc))
    ...
    post_layer_mm = []
    for pi in range(n_post):
        post_layer_mm.append(last_core_mm + (pi + 1) * pitch_post)

    def _layer_mm_for_index(i):
        if i < n_enc:
            return core_layer_mm[i] if i < len(core_layer_mm) else 0.0
        pi = i - n_enc
        return post_layer_mm[pi] if pi < len(post_layer_mm) else last_core_mm

    for i, ly in enumerate(layers[:n_capas]):
        try:
    ...
    if (
        ...
        n_enc_conf = n_enc if post_on else None
        stirrup_layer_indices, tie_layer_indices = cab.cabezal_confinement_layout_spec(
            n_capas, confinement_type, n_capas_encuentro=n_enc_conf,
        )
        pitch_frac = 0.12
        try:
        ...
        if (
            ...
            if post_on and n_post > 0 and post_layer_mm:
                ...
                if 0 <= last_post_li < len(layers):
                    try:
                        last_post_bar_d = float(
                            layers[last_post_li].get(u"bar_diam_mm") or bar_diam_mm,
                        )
                    except Exception:
                        pass
                clear_mm = (
                ...
                if layers:
                    try:
                        first_bar_d = float(
                            layers[0].get(u"bar_diam_mm") or bar_diam_mm,
                        )
                    except Exception:
                        pass
                clear_first_mm = (
            ...
            stirrup_layer_indices = list(range(n_capas))
        ...
        if (
            ...
            n_bars_cross = cab.cabezal_confinement_cross_tie_govern_n_bars(
                layers[:n_capas],
            )
