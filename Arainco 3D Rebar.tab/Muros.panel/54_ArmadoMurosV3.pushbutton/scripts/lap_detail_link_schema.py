# -*- coding: utf-8 -*-
"""
Almacenamiento extensible para vincular instancias de Detail Component (empalme/traslapo)
a dos barras (Rebar) consecutivas en un troceo. Usado por el DMU de actualización automática.
"""
...
LAP_DETAIL_LINK_SCHEMA_GUID = Guid("a1b2c3d4-e5f6-4789-a012-3456789abcd0")
_SCHEMA_NAME = "BIMToolsLapDetailLink"
_VENDOR_ID = "BIMT"
...
def _ensure_schema():
    ...
    sch = Schema.Lookup(LAP_DETAIL_LINK_SCHEMA_GUID)
    if sch is not None:
        _schema_cached = sch
        return sch
    sb = SchemaBuilder(LAP_DETAIL_LINK_SCHEMA_GUID)
...
def set_lap_detail_rebar_link(detail_inst, rebar_tail_id, rebar_head_id, dimension_id=None):
    """
    Guarda en la instancia de detalle los ElementId (como enteros) de las dos barras
    que delimitan el tramo de traslapo, y opcionalmente la cota asociada.
    """
...
def get_lap_detail_rebar_link(detail_inst):
    """
...
def iter_lap_linked_detail_instances(document):
    """
    Itera FamilyInstance (Detail Components) que tienen datos BIMToolsLapDetailLink.
    Yields: (FamilyInstance, link_dict)
    """
    if document is None:
        return
    try:
    ...
    try:
        ...
        for el in col:
            try:
                link = get_lap_detail_rebar_link(el)
            except Exception:
                link = None
            if link is not None:
                yield el, link
...
def find_lap_details_touching_rebar_ids(document, id_ints):
    """
    id_ints: iterable de IntegerValue de rebars tocados o borrados.
    Retorna lista de (FamilyInstance, link_dict) donde ra o rb coinciden.
    """
    if not id_ints:
        return []
    want = set(int(x) for x in id_ints)
    out = []
    for inst, link in iter_lap_linked_detail_instances(document):
