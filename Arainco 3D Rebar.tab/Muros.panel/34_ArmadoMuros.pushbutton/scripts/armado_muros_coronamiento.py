...
def _crear_coronamiento_voladizos_stack(doc, walls_ord, bar_type_fallback, res):
...
def aplicar_etiquetado_coronamiento(doc, cor_res, uidoc=None):
    ...
    try:
        ...
        pass
    try:
        tag_res = _cab_tags.etiquetar_cabezal_longitudinales_en_vista(
            doc, view, tag_ids, tag_meta=tag_meta,
        )
...
def _crear_coronamiento_superior(doc, wall, bar_type, diam_mm, res):
    ...
    res[u"messages"].append(
        u"Coronamiento sup. muro Id {0}: {1}Ø{2} mm ({3}).".format(
            res.get(u"host_wall_id") or u"?",
            int(res[u"n_bars_spec"]),
            int(diam_mm),
            _coronamiento_extremos_resumen(doc, wall),
        ),
    )
...
def _crear_coronamiento_inferior_pie_muro(doc, wall, bar_type, diam_mm, res):
...
def _crear_coronamiento_inferior_muro(doc, wall, bar_type, diam_mm, res):
