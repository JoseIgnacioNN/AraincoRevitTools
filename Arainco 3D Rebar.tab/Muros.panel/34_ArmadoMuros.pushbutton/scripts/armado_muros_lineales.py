...
def _aplicar_etiquetas_malla_todos(
    ...
    cover_asignados=0, muro_contencion=False,
):
...
def aplicar_unobscured_armado_muros_en_vista(
    ...
    - **Cabezal / confinamiento / coronamiento**: ``SetUnobscuredInView(True)`` (+ sólido).
    """
    vacio = {
...
def cota_inferior_muro_metros_aprox(wall):
    """Para UI: cota aproximada de la cara inferior del muro (metros)."""
    return _wall_bottom_z_sort_key_ft(wall) * 0.3048


def cota_superior_muro_metros_aprox(wall):
    """Para UI: cota aproximada de la cara superior del muro (metros)."""
    try:
        bb = wall.get_BoundingBox(None)
        if bb is not None:
            return float(bb.Max.Z) * 0.3048
    except Exception:
        pass
    return cota_inferior_muro_metros_aprox(wall)
...
def _aplicar_etiquetado_coronamiento_todos(doc, uidoc, embed_resumen, errores):
