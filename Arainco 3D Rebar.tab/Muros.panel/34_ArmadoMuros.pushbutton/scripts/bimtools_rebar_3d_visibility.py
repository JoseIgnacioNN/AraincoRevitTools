# -*- coding: utf-8 -*-
u"""
Visibilidad de armadura en vistas (View Unobscured / sólido en vista).

Copia portable de ``BIMTools.extension/scripts/bimtools_rebar_3d_visibility.py``.
"""
...
def _apply_rebar_visibility_in_view(doc, rebars, view, unobscured, solid_in_view=None):
    """``SetUnobscuredInView`` + ``SetSolidInView`` en ``Rebar`` para ``view``."""
    if not rebars or doc is None or view is None or not isinstance(view, View):
        return 0
    if solid_in_view is None:
        solid_in_view = unobscured
    n_ok = 0
