...
Revit 2024 + RPS (IronPython 3.4). Resuelve ``sys.path`` desde el directorio de este módulo
(copia portable en el pushbutton). Si la consola no define ``__file__``, opcionalmente
``RPS_SCRIPTS_DIR_OVERRIDE`` apunta a la carpeta que contiene ``rebar_extender_l_ganchos_135_rps.py``.
"""
...
# RPS a veces ejecuta sin __file__; opcional: ruta absoluta a la carpeta del pushbutton.
