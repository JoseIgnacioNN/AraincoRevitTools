# -*- coding: utf-8 -*-
"""Diálogos modales WPF (tema oscuro BIMTools) — inicio, resultado e información."""
...
_DIALOG = u"Arainco: Dividir rebar set Maximum Spacing"
...
def _bind_escape_close(win, on_close):
    def _on_key(sender, args):
        if args.Key == Key.Escape:
            on_close(sender, args)
            args.Handled = True
...
def show_selection_instructions(uiapp=None):
    ...
    return show_ok_cancel_dialog(
        _DIALOG,
        u"Divida un conjunto de barras con trazado Maximum Spacing en dos subconjuntos.",
        u"1. Seleccione el Rebar (shape-driven, separación máxima, al menos dos barras).\n"
        u"2. Seleccione la barra donde cortar (no la última del conjunto).\n\n"
        u"Pulse Aceptar para iniciar la selección en el modelo. Esc cancela en cualquier paso.",
        ok_text=u"Aceptar",
        cancel_text=u"Cancelar",
        hwnd_revit=hwnd,
        uiapp=uiapp,
