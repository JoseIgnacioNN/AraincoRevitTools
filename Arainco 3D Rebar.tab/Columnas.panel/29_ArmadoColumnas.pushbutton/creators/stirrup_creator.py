# -*- coding: utf-8 -*-
u"""
Creación de estribos y trabas para columnas.

REGLA DE CAPA:
- Requiere una transacción Revit activa al llamar a ``create_stirrups_for_column``.
- ``run_stirrup_layout`` abre su propio TransactionGroup + Transaction.
- No importa módulos de ui/.

Portado literalmente desde scripts/column_stirrup_creator.py con la adición
de ``run_stirrup_layout`` que coordina la creación en lote.
"""
import clr
...
# ---------------------------------------------------------------------------
# Geometría de sólidos (duplicada aquí para mantener auto-contención)
# ---------------------------------------------------------------------------
...
def _section_lx_ly_lz_orthogonal(column, trans):
    u"""Ejes locales ⟂: lz según línea del pilar, lx proyectado desde BasisX."""
    try:
...
def stirrup_pattern_options(val):
    if val == 2:
        return [u"Perimetral \u00dcnicamente"]
    elif val == 3:
        return [u"Perimetral + Traba (Rojo)", u"Perimetral \u00dcnicamente"]
    elif val == 4:
        return [
            u"Perimetral + Estribo Int. (Verde)",
            u"Perimetral + Traba (Rojo)",
            u"Perimetral \u00dcnicamente",
        ]
    elif val == 5:
        return [
            u"Perimetral + Estribo Int. (Verde)",
...
def tie_axis_shift_toward_section_center(
    center_coord, long_bar_radius, tie_index=None, bar_count=None
):
...
# ---------------------------------------------------------------------------
# Geometría de sección (usada por jobs.py para alinear rejilla)
# ---------------------------------------------------------------------------

def column_bar_geometry(col, stirrup_bar_type=None, cover_mm=25.0, long_bar_diam_mm=16.0):
    u"""
    Devuelve ``(plan_anchor, lx, ly, sa, sb, offset_long_ft)``
    donde offset_long_ft = cover + Ø_estribo + radio_long.
    """
    try:
...
# ---------------------------------------------------------------------------
# Creación de estribos para una columna (requiere transacción activa)
# ---------------------------------------------------------------------------
...
def create_stirrups_for_column(
    ...
    try:
        ...
        rect_defs, tie_defs = build_stirrup_rect_and_tie_defs(
            val_a, val_b, sel_a_text, sel_b_text
        )
        ...
        for idx, is_a in tie_defs:
            if is_a:
                x_tie = tie_axis_shift_toward_section_center(
                    _center_along_a(idx), long_bar_r_ft,
                    tie_index=idx, bar_count=val_a
                )
                pt1 = (
                    plan_anchor_rebar
                    + lx.Multiply(x_tie)
                ...
                y_tie = tie_axis_shift_toward_section_center(
                    _center_along_b(idx), long_bar_r_ft,
                    tie_index=idx, bar_count=val_b
                )
                pt1 = (
                    plan_anchor_rebar
...
# ---------------------------------------------------------------------------
# Punto de entrada de capa: layout en lote con TransactionGroup
# ---------------------------------------------------------------------------

def _resolve_hook_135_type(doc):
    """Busca el primer RebarHookType con '135' en el nombre; None si no encuentra."""
    try:
        coll = FilteredElementCollector(doc).OfClass(RebarHookType)
        for ht in coll:
            try:
                nm = ht.Name.upper()
                if u"135" in nm or u"135\u00b0" in nm:
                    return ht
            except Exception:
                continue
        for ht in coll:
...
def run_stirrup_layout(doc, columns_ordered, dims_cache, wiz):
    """
    Crea todos los estribos del wizard en un TransactionGroup.

    ``wiz`` debe exponer:
    - ``stirrup_config_by_section_key``: dict {(s_mm, L_mm): StirrupConfig}
    ...
    from core.geometry import _canonical_section_mm_key, stirrup_bar_type_override, stirrup_spacing_override

    hook_type = _resolve_hook_135_type(doc)
    if hook_type is None:
        TaskDialog.Show(
            u"Armado Columnas",
            u"No se encontró RebarHookType de 135°. No se crearon estribos.",
        )
        return

    errors = []
    tg = TransactionGroup(doc, u"Arainco: Armado Columnas — Estribos")
    tg.Start()
    try:
        for col in columns_ordered or []:
            ...
            if iv < 0:
                continue
            dims = dims_cache.get(iv)
            if dims is None:
                continue
            width, depth = dims[0], dims[1]
            sk = _canonical_section_mm_key(width, depth)
            cfg = wiz.stirrup_config_by_section_key.get(sk)
            if cfg is None:
                continue
            sbt = stirrup_bar_type_override(wiz.stirrup_bar_type_by_column_id, col)
            if sbt is None:
                sbt = getattr(cfg, "stirrup_bar_type", None)
            if sbt is None:
                continue
            spacing_mm = stirrup_spacing_override(wiz.stirrup_spacing_by_column_id, col)
            if spacing_mm is None:
                spacing_mm = float(getattr(cfg, "spacing_mm", 150.0))
            val_a     = int(getattr(cfg, "bars_a",       4))
            val_b     = int(getattr(cfg, "bars_b",       4))
            sel_a     = getattr(cfg, "pattern_a_text", u"Perimetral \u00dcnicamente")
            sel_b     = getattr(cfg, "pattern_b_text", u"Perimetral \u00dcnicamente")
            long_diam = float(getattr(wiz, "global_long_bar_diam_mm", 16.0))

            t = Transaction(doc, u"Arainco: Armado Columnas — Estribo columna")
            t.Start()
            try:
                create_stirrups_for_column(
                    doc, col, val_a, val_b, sel_a, sel_b,
                    sbt, hook_type, spacing_mm,
                    cover_mm=25.0,
                    long_bar_diam_mm=long_diam,
                )
                t.Commit()
            except Exception as ex:
                ...
                try:
                    ...
                    pass
                try:
                    errors.append(u"Columna {}: {}".format(iv, ex))
                except Exception:
                    errors.append(u"Columna {}: error".format(iv))
    ...
    if errors:
        TaskDialog.Show(
            u"Armado Columnas — Estribos",
            u"Se crearon estribos con los siguientes errores:\n\n" + u"\n".join(errors),
        )
