...
Las etiquetas se crean **sin leader**, en orientación **horizontal**, con la cabecera en el
**centro del ``ArrayLength``** del layout (eje de la viga) y **centrada en el canto**
...
from Autodesk.Revit.DB import IndependentTag, TagMode, TagOrientation, XYZ
...
def _etiquetar_rebar_confinamiento_viga(
    document, view, rebar, tag_map, tags_mod=None, host=None,
):
...
def _etiquetar_multihost_confinamiento_viga(
    document, view, rebar_ids, tag_map, tags_mod, host=None,
):
...
def _tag_single_confinement(document, view, res, job, tags_mod, tag_map):
    """Etiqueta estribo/traba: sin leader, centrada en array + canto de viga."""
...
def _flush_multihost_stirrup_viga(document, view, res, tag_map, tags_mod):
    ...
    for key, grp in pending.items():
        ...
        ok, err = _etiquetar_multihost_confinamiento_viga(
            document, view, ids, mh_map, tags_mod, host=host,
        )
        if ok:
            n_ok += 1
        else:
...
def _flush_multihost_confinamiento_viga(document, view, res, tag_map, tags_mod):
...
def etiquetar_confinamiento_en_vista(
    ...
    for job in ordered:
        ...
        ok_tag, err_tag = _tag_single_confinement(
            document, view, res, job, tags_mod, tag_map,
        )
        if ok_tag:
            n_ok += 1
        else:
    ...
    if pending_traba:
        try:
            _flush_multihost_confinamiento_viga(
                document, view, res, tag_map, tags_mod,
            )
