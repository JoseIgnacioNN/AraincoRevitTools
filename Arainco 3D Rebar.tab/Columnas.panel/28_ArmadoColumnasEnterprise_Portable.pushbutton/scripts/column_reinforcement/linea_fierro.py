...
# Etiquetas del despiece en lienzo: parciales al ceiling del múltiplo más próximo (10 mm).
...
def _despiece_parcial_mm_ceiling_10(mm):
    """Ceiling al múltiplo de ``DESPIECE_LIENZO_LEN_ROUND_MM`` (solo texto en lienzo)."""
    try:
        v = float(mm)
    except Exception:
        return 0
...
def etiqueta_despiece_mm(diameter_mm, fp):
    """
    Etiqueta de despiece en lienzo: ``ø12 L=9190`` o ``ø12 L=total (parcial+…)``.

    Cada largo parcial se redondea al **ceiling** del múltiplo de 10 mm más próximo;
    ``L=total`` es la **suma** de esos parciales (no el ceiling de la suma cruda).
    """
