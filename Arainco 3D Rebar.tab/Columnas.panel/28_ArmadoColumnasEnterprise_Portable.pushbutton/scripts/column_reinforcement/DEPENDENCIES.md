...
**Empaquetados junto a `<pushbutton>/scripts/`** (autocontenidos para copiar el botón a otra extensión):
`column_stirrup_creator.py`, `column_stirrup_tags.py`, `confinement_dim_link_schema.py`,
`confinement_dim_updater_dmu.py`, `armado_columnas_instruction_dialog.py`,
`conjunto_guid.py`, `armado_muros_cabezal_tags.py`, `armado_vigas/revit/etiquetar_confinamiento.py`,
`bimtools_wpf_dark_theme.py`, `revit_wpf_window_position.py`
(mismos nombres que en la raíz canónica o en Armado vigas/muros; sincronizar al publicar).

**Bootstrap portable** (raíz del `.pushbutton`): `bimtools_access_bootstrap.py` — no requiere
`BIMTools.tab/`; si no hay `scripts/corporate_access.py` en la extensión destino, omite la guardia.

## Dependencias opcionales de runtime (fuera del pushbutton)

| Módulo | Uso | Sin él |
|--------|-----|--------|
| `enfierrado_shaft_hashtag` | Respaldo de mapas/refs de etiqueta en `column_stirrup_tags` / `etiquetar_confinamiento` | Etiquetado usa `armado_muros_cabezal_tags` empaquetado |
| `corporate_access.py` + `bimtools_script_guard` | Guardia en despliegue BIMTools completo | Omitido en copia portable a otra extensión |
| Familias Revit del modelo | Tags `EST_A_STRUCTURAL REBAR TAG_CONFINAMIENTO_VIGA*`, cota `Linear - Confinamiento` | Errores informativos en UI; armado de barras sigue |
