...
from armado_vigas.domain.constants import ESTRIBO_INSET_MM, ESTRIBO_SPACING_MIN
...
def compute_stirrup_zones(beam):
    ...
    sp_e = max(ESTRIBO_SPACING_MIN, int(beam.get("estExtSpacing") or 125))
    sp_c = max(ESTRIBO_SPACING_MIN, int(beam.get("estCentSpacing") or 200))
