...
from armado_vigas.domain.constants import BAR_COUNT_MIN, BAR_COUNT_MAX, CAPAS_MAX, LONG_DIAM_OPTS
...
class ArmadoVigasCanvasView(object):
    def __init__(self, win, callbacks):
        ...
        callbacks: dict con claves
          ...
          on_select_tramo(tramo_id), on_select_beam(idx),
    ...
    def _redraw_impl(self, session):
        ...
        layout_result = lay.compute_layout(beams, viewport_w)
        ...
        if self.selected_beam_idx >= len(beams):
            self.selected_beam_idx = -1
    ...
    def _show_empty(self, apoyos_loaded):
        ...
        if self._txt_sel:
            self._txt_sel.Text = (
                u"Clic viga → selección · Traslape sup/inf en panel derecho · "
    ...
    def _update_headers(self, beams, tramos_sup, tramos_inf, apoyos_loaded, layout_result):
        ...
        if self._txt_sel:
            base = (
                ...
                u"Controles Tn en bandas alzado · Cent/Ext L auto en panel sección."
                if apoyos_loaded
                else u"Clic viga → selección · Traslape en panel derecho · Cent/Ext · Suple en canvas."
            )
    ...
    def _draw_elevation_beam_run(self, cnv, run_left, run_w):
        ...
        axis = brush_hex(u"#64748b", 140)
        self._elev_rect(
            cnv, run_left, _ELEV_BEAM_TOP, run_w, _ELEV_BEAM_H,
            fill=SolidColorBrush(Color.FromArgb(18, 226, 232, 240)),
            zindex=1,
        )
        ...
        self._elev_line(
            cnv, run_left, _ELEV_BEAM_TOP + _ELEV_BEAM_H * 0.5,
            run_left + run_w, _ELEV_BEAM_TOP + _ELEV_BEAM_H * 0.5,
            axis, 1.25, dash=[4.0, 2.5], zindex=2,
        )

    def _draw_elevation_direction_marker(self, cnv, left, width, order_idx, axis_reversed=False):
        """Marca LocationCurve 0 (⊥) → 1 (flecha), corta y centrada en el tramo."""
        y = _ELEV_BEAM_TOP + _ELEV_BEAM_H * 0.5
        cx = left + width * 0.5
    ...
    def _draw_elevation_support(self, cnv, x, pt, idx):
        is_wall = unicode(pt.get("id", u"")).startswith(u"M")
        col_w = _ELEV_WALL_W if is_wall else _ELEV_COL_W
        col_top = _ELEV_COL_TOP + (6.0 if is_wall else 0.0)
        col_h = _ELEV_COL_H - (10.0 if is_wall else 0.0)
        left = x - col_w * 0.5
        fill = brush_hex(u"#34d399", 56) if is_wall else brush_hex(u"#94a3b8", 82)
        edge = brush_hex(u"#34d399", 180) if is_wall else brush_hex(u"#64748b", 190)
        self._elev_rect(cnv, left, col_top, col_w, col_h, fill=fill, stroke=edge, thickness=1.0, zindex=5)
        edge_top = _ELEV_BEAM_TOP
        edge_bot = _ELEV_BEAM_BOT
        self._elev_line(cnv, left, edge_top, left, edge_bot, edge, 1.25, dash=[2.5, 2.0], zindex=6)
        self._elev_line(cnv, left + col_w, edge_top, left + col_w, edge_bot, edge, 1.25, dash=[2.5, 2.0], zindex=6)
        self._elev_line(
            cnv, x, col_top, x, col_top + col_h, edge, 1.25, dash=[4.0, 2.0, 1.0, 2.0], zindex=7,
        )
        if not is_wall and idx > 0:
            pocket_w = min(20.0, col_w + 6.0)
            px = x - pocket_w * 0.5
            py = _ELEV_BEAM_TOP
            dash = brush_hex(u"#94a3b8", 200)
            self._elev_line(cnv, px, py, px, py + _ELEV_POCKET_D, dash, 1.5, zindex=8)
            self._elev_line(cnv, px, py + _ELEV_POCKET_D, px + pocket_w, py + _ELEV_POCKET_D, dash, 1.5, dash=[2.5, 2.0], zindex=8)
            self._elev_line(cnv, px + pocket_w, py + _ELEV_POCKET_D, px + pocket_w, py, dash, 1.5, zindex=8)
    ...
    def _support_col_specs(self, chain, content_w):
        specs = []
    ...
    def _draw_elevation_bottom_bars(self, cnv, layouts, tramos_inf, content_w, chain):
        specs = self._support_col_specs(chain, content_w) if chain else []
    ...
    def _draw_elevation_suple_tramos(self, cnv, beams, layouts, content_w):
    ...
    def _build_elevation_canvas(self, beams, layouts, tramos_sup, tramos_inf, session, apoyos_loaded, content_w):
        ...
        chain = lay.build_support_chain(beams, layouts) if apoyos_loaded else []
        run_left = 0.0
        run_w = content_w
        if layouts:
        ...
        for j, pt in enumerate(chain):
            x = lay.pct_to_px(pt["pct"], content_w)
            self._draw_elevation_support(cnv, x, pt, j)

        self._draw_elevation_top_bars(cnv, layouts, tramos_sup, content_w)
        self._draw_elevation_bottom_bars(cnv, layouts, tramos_inf, content_w, chain)
        self._draw_elevation_suple_tramos(cnv, beams, layouts, content_w)
        ...
        for i, beam in enumerate(beams):
            ...
            self._draw_elevation_direction_marker(
                cnv, left, width, i, bool(beam.get("axisReversed")),
            )
            ...
            is_sel = self.selected_beam_idx == i

            span = Border()
            span.Width = width
            span.Height = _ELEV_BEAM_H
            Canvas.SetLeft(span, left)
            Canvas.SetTop(span, _ELEV_BEAM_TOP)
            ...
            def _click_beam(sender, args, idx=idx_cap, b=beam_cap):
                self._select_beam_from_elevation(idx, b)
    ...
    def _build_labels(self, beams, layouts, session, apoyos_loaded, content_w):
        ...
        for i, beam in enumerate(beams):
            ...
            tb = TextBlock()
            tb.TextAlignment = TextAlignment.Center
            tb.Foreground = brush_hex(u"#7eb8d0") if self.selected_beam_idx == i else brush_hex(u"#95B8CC")
    ...
    def _section_at_row(self, beam, role, half=False):
        ...
        if role in (u"ext", u"uni"):
            ...
            spacing_val = beam.get("estCentSpacing") or 200
            diam_cb = make_diam_combo(
                self._win, diam_val, ESTRIBO_DIAM_OPTS,
    ...
    def _attach_section_zone_select(self, panel, idx, role):
        def _select(sender, args, i=idx, r=role):
            self.selected_stirrup_zone = {u"idx": i, u"role": r}
            self.selected_beam_idx = i
            self._cb.get("on_select_stirrup_zone", lambda _i, _r: None)(i, r)
            self._cb.get("on_redraw", lambda: None)()
    ...
    def _section_ctrl_zone(
        ...
        sel = (
            self.selected_stirrup_zone
            and self.selected_stirrup_zone.get("idx") == idx
            and self.selected_stirrup_zone.get("role") == role
        )
    ...
    def _build_suple_canvas_slot(self, beam, idx, width, tramo_beams=None):
        ...
        selected = self.selected_beam_idx == idx
        ...
        def _select_suple_slot(sender, args, i=idx):
            self.selected_beam_idx = i
            self.selected_stirrup_zone = {"idx": i, "role": u"suple"}
            self._cb.get("on_select_stirrup_zone", lambda _i, _r: None)(i, u"suple")
            self._cb.get("on_redraw", lambda: None)()
    ...
    def _confin_section_block(self, beam, idx):
        ...
        sel = (
            self.selected_stirrup_zone
            and self.selected_stirrup_zone.get("idx") == idx
            and self.selected_stirrup_zone.get("role") == u"confin"
        )
    ...
    def _laterales_section_block(self, session, idx):
        ...
        sel = (
            self.selected_stirrup_zone
            and self.selected_stirrup_zone.get("idx") == idx
            and self.selected_stirrup_zone.get("role") == u"laterales"
        )
    ...
    def _suple_inf_global_row(self, beam, idx, tramo_beams, full_width=False):
        ...
        sel = (
            self.selected_stirrup_zone
            and self.selected_stirrup_zone.get("idx") == idx
            and self.selected_stirrup_zone.get("role") == u"suple"
        )
        wrap = Border()
    ...
    def _zone_panel(self, beam, idx, role, zone, plan, show_len_hint=False, stacked=False):
        ...
        if role in ("ext", "uni"):
            ...
            row_idx += 1
            self._zone_add_field(grid, row_idx, u"@", make_spacing_combo(
                self._win, beam.get("estExtSpacing") or 125,
                lambda v, b=beam: self._set_beam_field(b, "estExtSpacing", v),
                compact=True,
            ))
            row_idx += 1
        ...
        if role == "cent":
            ...
            row_idx += 1
            self._zone_add_field(grid, row_idx, u"@", make_spacing_combo(
                self._win, beam.get("estCentSpacing") or 200,
                lambda v, b=beam: self._set_beam_field(b, "estCentSpacing", v),
                compact=True,
            ))
            row_idx += 1
        ...
        def _select(sender, args, i=idx, r=role):
            self.selected_stirrup_zone = {"idx": i, "role": r}
            self.selected_beam_idx = i
            self._cb.get("on_select_stirrup_zone", lambda _i, _r: None)(i, r)
            self._cb.get("on_redraw", lambda: None)()
    ...
    def _build_empalme_button_row(self, beams, layouts, session, content_w, face):
        ...
        for i, beam in enumerate(beams):
            ...
            def _click_empalme(sender, args, i=idx_cap, bid=beam_id, f=face):
                self.selected_beam_idx = i
                self._cb.get("on_toggle_empalme", lambda _b, _f: None)(bid, f)
                self._cb.get("on_select_beam", lambda _i: None)(i)
                self._cb.get("on_redraw", lambda: None)()
