# -*- coding: utf-8 -*-
"""Arainco: Area Rein. losa (Sketch) — entrada pyRevit fina.

Lógica en ``BIMTools.extension/scripts/area_rein_losa_sketch.py``.
Independiente de Malla en losa / ``area_reinforcement_losa``.
"""
...
_MAIN_FILE = u"area_rein_losa_sketch.py"
...
if _bimtools_access.require_tool_access(__file__, __revit__, __title__):
    ...
    try:
        if u"area_rein_losa_sketch" in sys.modules:
            try:
                del sys.modules[u"area_rein_losa_sketch"]
            except Exception:
                pass
        from area_rein_losa_sketch import run_pyrevit
        ...
        try:
            TaskDialog.Show(
                _DIALOG_TITLE,
                u"Error al iniciar Area Rein. losa:\n\n{0}".format(_err[-1800:]),
            )
        except Exception:
            pass
        raise
