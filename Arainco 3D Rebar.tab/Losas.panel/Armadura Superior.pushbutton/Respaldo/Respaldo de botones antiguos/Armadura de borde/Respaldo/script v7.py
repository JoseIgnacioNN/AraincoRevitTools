...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def __init__(self, xaml_file, datos):
        ...
        if datos.barra_izq is not None:
            self.chkIzquierda.IsChecked = datos.barra_izq
            self.chkDerecha.IsChecked = not datos.barra_izq
    ...
    def guardar_estado(self):
        ...
        estado.barra_izq = self.chkIzquierda.IsChecked
        ...
        # Rescatar Lext del campo de solo lectura
    ...
    def LextClick(self, sender, args):
        self.action = "lext" # Indica que se hizo clic en el botón "Seleccionar" de extensión
        self.guardar_estado()
        self.Close()
    ...
    def AplicarClick(self, sender, args):
        ...
        if not self.txtL.Text:
            forms.alert("Debes definir el largo de barra.", title="Falta información")
            return
        try:
            if float(self.txtL.Text) <= 0:
                forms.alert("El largo de barra debe ser mayor que cero.", title="Valor inválido")
                return
        except ValueError:
            forms.alert("El largo de barra debe ser un número válido.", title="Valor inválido")
            return
...
while True:
    ...
    if form.action == "aplicar":
        ...
        # Transaction Group
        t_group = DB.TransactionGroup(doc, "ARAINCO - Armadura Superior de borde")
        t_group.Start()
...
# ===================================================================================
        ...
        if estado.barra_izq == True:
            start = p1_3D + v_bar_3D * L
            end = p1_3D + v_bar_3D * offset_lateral
            gancho_izq = None
            gancho_der = gancho
        else:
            start = p1_3D - v_bar_3D * offset_lateral # Extremo inicial de la barra
            end = p1_3D - v_bar_3D * L # Extremo final de la barra
            gancho_izq = gancho
            gancho_der = None
