...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def __init__(self, xaml_file, datos):
        ...
        if datos.L is not None:
            self.txtL.Text = "{:.0f}".format(datos.L)
        ...
        if datos.gancho_izq is not None:
            self.chkIzquierda.IsChecked = datos.gancho_izq
            self.chkDerecha.IsChecked = not datos.gancho_izq
    ...
    def guardar_estado(self):
        ...
        estado.gancho_izq = self.chkIzquierda.IsChecked

        # Rescatar L (si el usuario lo sobreescribió manualmente)
        if self.txtL.Text:
            try: estado.L = float(self.txtL.Text)
    ...
    def ChkIzquierda_Click(self, sender, args):
        self.chkIzquierda.IsChecked = True
        self.chkDerecha.IsChecked = False

    def ChkDerecha_Click(self, sender, args):
            self.chkIzquierda.IsChecked = False
            self.chkDerecha.IsChecked = True
    ...
    def AplicarClick(self, sender, args):
        ...
        # Validación L
        if not self.txtL.Text:
            forms.alert("Debes definir el largo de la barra.", title="Falta información")
            return
        try:
            float(self.txtL.Text)
        except ValueError:
            forms.alert("El valor de L debe ser un número válido.", title="Valor inválido")
            return
...
while True:
    ...
    if form.action == "vano":
        ...
        estado.largo = UnitUtils.ConvertFromInternalUnits(((p2.X - p1.X) ** 2 + (p2.Y - p1.Y) ** 2) ** 0.5, UnitTypeId.Millimeters)
    ...
    if form.action == "aplicar":
        L = UnitUtils.ConvertToInternalUnits(estado.L, UnitTypeId.Millimeters)
...
# ===================================================================================
        ...
        # Desfase para la detección de la losa host (evita host erróneo en bordes donde llegan losas con distinto nivel)
        mid_point = DB.XYZ((p1.X + p2.X) / 2, (p1.Y + p2.Y) / 2, p1.Z) # Punto matemático central del recorrido
        v_trazo_2d = DB.XYZ(p2.X - p1.X, p2.Y - p1.Y, 0).Normalize() # Vector del recorrido 2D
        v_izq_2d = DB.XYZ(-v_trazo_2d.Y, v_trazo_2d.X, 0) # Vector perpendicular hacia la izquierda (Rotación de 90° en 2D)
        if estado.gancho_izq == True:
        ...
        pt_min = DB.XYZ(pt_busqueda.X - 0.0164, pt_busqueda.Y - 0.0164, Z_elev - 6.562) # Esquina inferior izquierda del pilar
        pt_max = DB.XYZ(pt_busqueda.X + 0.0164, pt_busqueda.Y + 0.0164, Z_elev + 6.562) # Esquina superior derecha del pilar
        ...
        p_test = DB.XYZ(pt_busqueda.X, pt_busqueda.Y, Z_elev) # Punto usado para generar láser de proyección hacia la cara de la losa candidata
        dist_min = float('inf')
        slab = None
...
# ===================================================================================
        ...
        p1 = p1 + v_trazo * offset_lateral # Punto inicial del recorrido, reducido por el recubrimiento lateral del borde de la losa
        ...
        if estado.gancho_izq == True:
            start = p1_3d - v_bar * offset_lateral # Extremo inicial de la barra
            end = p1_3d - v_bar * L # Extremo final de la barra
            gancho_izq = gancho
            gancho_der = None
        else:
            start = p1_3d + v_bar * L
            end = p1_3d + v_bar * offset_lateral
            gancho_izq = None
            gancho_der = gancho
...
# ===================================================================================
    ...
    # Barra individual
            rebar = DB.Structure.Rebar.CreateFromCurves(
                ...
                gancho_izq, gancho_der, # Gancho inicial y final
                slab, # Anfitrión
    ...
    # Offset para Multi-Rebar Annotation y Tag
            ...
            if estado.gancho_izq == True:
                offset_mra = UnitUtils.ConvertToInternalUnits(-75, UnitTypeId.Centimeters)
                offset_tag = UnitUtils.ConvertToInternalUnits(-80, UnitTypeId.Centimeters)
            else:
                offset_mra = UnitUtils.ConvertToInternalUnits(75, UnitTypeId.Centimeters)
                offset_tag = UnitUtils.ConvertToInternalUnits(80, UnitTypeId.Centimeters)
