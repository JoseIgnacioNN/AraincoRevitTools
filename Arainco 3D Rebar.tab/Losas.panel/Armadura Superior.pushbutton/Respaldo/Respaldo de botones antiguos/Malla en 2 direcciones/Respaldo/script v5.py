...
def seleccionar_puntos():
    ...
    def capturador(sender, args):
        for elem_id in args.GetAddedElementIds(): # Recorremos todos los elementos creados
            if elem_id not in id_capturado:
                id_capturado.append(elem_id)
        if len(id_capturado) >= 2 and not teclas_enviadas[0]: # Solo disparamos el ESC cuando ya tenemos la cantidad de líneas solicitadas
    ...
    linea_1_id = id_capturado[0] # Id de la primera línea dibujada
    linea_2_id = id_capturado[1] # Id de la segunda línea dibujada
    p1 = doc.GetElement(linea_1_id).Location.Curve.GetEndPoint(0)
    p2 = doc.GetElement(linea_1_id).Location.Curve.GetEndPoint(1)
    p3 = doc.GetElement(linea_2_id).Location.Curve.GetEndPoint(0)
    p4 = doc.GetElement(linea_2_id).Location.Curve.GetEndPoint(1)
...
while True:
    ...
    if form.action == "aplicar":
        ...
        try: p1_vano1, p2_vano1, p1_vano2, p2_vano2 = seleccionar_puntos()
        except OperationCanceledException: continue

        # Transaction Group
        t_group = DB.TransactionGroup(doc, "ARAINCO - Armadura Superior borde a borde (1 dirección)")
        t_group.Start()
...
# ===================================================================================
        ...
        if (p2_vano1-p1_vano1).GetLength() <= (2 * offset_lateral) or (p2_vano2-p1_vano2).GetLength() <= (2 * offset_lateral):
            t_group.RollBack()
        ...
        if abs(v_vano1.DotProduct(v_vano2)) > 0.001: # Si el valor absoluto es mayor a la tolerancia, NO son perpendiculares
            t_group.RollBack()
            forms.alert("El recorrido debe ser perpendicular al vano. Aplica el recorrido nuevamente.", title="Recorrido no válido")
            continue

        # Buscar el tipo de gancho
        nombre_gancho = "Standard - 180 deg."
        tipos_gancho = DB.FilteredElementCollector(doc).OfClass(DB.Structure.RebarHookType).ToElements()
        gancho = next((h for h in tipos_gancho if h.get_Parameter(DB.BuiltInParameter.SYMBOL_NAME_PARAM).AsString() == nombre_gancho), None)
        if not gancho:
            t_group.RollBack()
            forms.alert("No se encontró el gancho '{}' en el proyecto.\n\nRevisa que esté cargado en el proyecto o que el nombre esté escrito exactamente igual.".format(nombre_gancho), title="Gancho no encontrado")
            continue

        centro_vano_2D = (p1_vano1 + p2_vano1) / 2
        p1_vano1_z = get_z_on_plane(p1_vano1.X, p1_vano1.Y, origen_desplazado, plane_normal)
        p2_vano1_z = get_z_on_plane(p2_vano1.X, p2_vano1.Y, origen_desplazado, plane_normal)
        p1_vano1_3D = DB.XYZ(p1_vano1.X, p1_vano1.Y, p1_vano1_z)
        p2_vano1_3D = DB.XYZ(p2_vano1.X, p2_vano1.Y, p2_vano1_z)
        L = (p2_vano1_3D - p1_vano1_3D).GetLength() # Largo de la barra
        centro_vano_3D = (p1_vano1_3D + p2_vano1_3D) /2
        v_bar_3D = (p2_vano1_3D - p1_vano1_3D).Normalize() # Vector director de la barra (en la dirección longitudinal)
        ...
        # Asegurarnos de que el producto cruz no haya invertido la dirección de avance hacia p2_vano2
        v_recorrido_2D = (p2_vano2 - p1_vano2).Normalize()
        ...
        p1_z = get_z_on_plane(p1_vano2.X, p1_vano2.Y, origen_desplazado, plane_normal)
        p2_z = get_z_on_plane(p2_vano2.X, p2_vano2.Y, origen_desplazado, plane_normal)
        p1_3D = DB.XYZ(p1_vano2.X, p1_vano2.Y, p1_z)
        p2_3D = DB.XYZ(p2_vano2.X, p2_vano2.Y, p2_z)
        desplazamiento = (p1_3D - centro_vano_3D).DotProduct(v_recorrido_3D)
        centro_barra = centro_vano_3D + v_recorrido_3D * (desplazamiento + offset_lateral)
        ...
        L_recorrido = (p2_3D - p1_3D).DotProduct(v_recorrido_3D) - 2 * offset_lateral # Largo del recorrido (proyectado perpendicular al eje de la barra)
...
# ===================================================================================
    ...
    # Barra individual
            rebar = DB.Structure.Rebar.CreateFromCurves(
                ...
                gancho, gancho, # Gancho inicial y final
                slab, # Anfitrión
    ...
    # Multi-Rebar Annotation
            ...
            if tipo:
                ...
                mid_point = (p1_vano1 + p2_vano1) / 2 # Se centra la cota al punto medio del recorrido
    ...
    # Tag
            ...
            tag_pos = DB.XYZ(centro_barra.X, centro_barra.Y, Z_elev)
