...
class ArmaduraSuperiorForm(forms.WPFWindow):  # Funciones del formulario
    ...
    def __init__(self, xaml_file, datos):
        ...
        if datos.largo1_cm is not None: 
            self.txtLargo1.Text = "{:.0f}".format(datos.largo1_cm)
        if datos.L1_cm is not None:     
            self.txtL1.Text = "{:.0f}".format(datos.L1_cm)
        if datos.largo2_cm is not None: 
            self.txtLargo2.Text = "{:.0f}".format(datos.largo2_cm)
        if datos.L2_cm is not None:     
            self.txtL2.Text = "{:.0f}".format(datos.L2_cm)
    ...
    def guardar_estado(self):
        ...
        if self.txtL1.Text:
            try:
                estado.L1_cm = float(self.txtL1.Text)
                estado.L1_v1 = UnitUtils.ConvertToInternalUnits(float(self.txtL1.Text), UnitTypeId.Centimeters)
            except ValueError:
                pass
        ...
        if self.txtL2.Text:
            try:
                estado.L2_cm = float(self.txtL2.Text)
                estado.L2_v2 = UnitUtils.ConvertToInternalUnits(float(self.txtL2.Text), UnitTypeId.Centimeters)
            except ValueError:
                pass
    ...
    def AplicarClick(self, sender, args):
        ...
        try:
            ...
            forms.alert("Ingresa un espaciamiento válido en centímetros.", title="Valor inválido")
            return
...
while True:
    ...
    if form.action == "vano1":
        ...
        estado.largo1_cm = UnitUtils.ConvertFromInternalUnits(largo1, UnitTypeId.Centimeters)
        estado.L1_v1 = UnitUtils.ConvertToInternalUnits(math.ceil(UnitUtils.ConvertFromInternalUnits(largo1/3,UnitTypeId.Centimeters)),UnitTypeId.Centimeters) # Redondear al centímetro
        estado.L1_cm = UnitUtils.ConvertFromInternalUnits(estado.L1_v1, UnitTypeId.Centimeters)
    ...
    if form.action == "vano2":
        ...
        estado.largo2_cm = UnitUtils.ConvertFromInternalUnits(largo2, UnitTypeId.Centimeters)
        estado.L2_v2 = UnitUtils.ConvertToInternalUnits(math.ceil(UnitUtils.ConvertFromInternalUnits(largo2/3,UnitTypeId.Centimeters)),UnitTypeId.Centimeters)
        estado.L2_cm = UnitUtils.ConvertFromInternalUnits(estado.L2_v2, UnitTypeId.Centimeters)
        continue
    ...
    if form.action == "aplicar":
        ...
        esp = UnitUtils.ConvertToInternalUnits(float(estado.esp_text), UnitTypeId.Centimeters)
