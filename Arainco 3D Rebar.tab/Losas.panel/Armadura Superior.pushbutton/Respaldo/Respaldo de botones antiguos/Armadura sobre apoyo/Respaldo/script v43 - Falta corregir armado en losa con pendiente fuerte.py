...
# ===================================================================================
        ...
        start_z = get_z_on_plane(start_x, start_y, origen_desplazado, plane_normal)
        start = DB.XYZ(start_x, start_y, start_z) # Punto inicial de la barra
        end_x = p1.X + L2 * math.sin(ang)
        end_y = p1.Y - L2 * math.cos(ang)
        end_z = get_z_on_plane(end_x, end_y, origen_desplazado, plane_normal)
        end = DB.XYZ(end_x, end_y, end_z) # Punto final de la barra
        curves = [DB.Line.CreateBound(start, end)] # Las puntos que conforman la barra deben estar en una lista
        v_bar = (end - start).Normalize() # Vector director de la primera barra (en la dirección longitudinal de la barra)
        ...
        p1_z = get_z_on_plane(p1.X, p1.Y, origen_desplazado, plane_normal) # Elevación de la barra inicial
        p2_z = get_z_on_plane(p2.X, p2.Y, origen_desplazado, plane_normal) # Elevación de la barra final
        v_usuario_3d = (DB.XYZ(p2.X, p2.Y, p2_z) - DB.XYZ(p1.X, p1.Y, p1_z)).Normalize()
        if v_dir.DotProduct(v_usuario_3d) < 0:
            v_dir = -v_dir

        v_recorrido_real = DB.XYZ(p2.X, p2.Y, p2_z) - DB.XYZ(p1.X, p1.Y, p1_z) # Vector del recorrido real 
        L_recorrido = abs(v_recorrido_real.DotProduct(v_dir)) # Largo del recorrido, corregido por la distorsión que genera la losa con pendiente
...
# ===================================================================================
        ...
        # Multi-Rebar Annotation
                ...
                if tipo:
                    ...
                    tipo_opts.DimensionLineDirection = DB.XYZ(p2.X - p1.X, p2.Y - p1.Y, 0).Normalize() # Vector director 2D, en el plano de la planta
        ...
        # Tag
                ...
                centro_barra = centro_primera + v_dir * idx_central * esp_real # Centroide de la barra central
