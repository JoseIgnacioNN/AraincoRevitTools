...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def __init__(self, xaml_file, datos):
        ...
        if datos.ext_izq is not None:
            self.txtExtIzq.Text = "{:.0f}".format(datos.ext_izq)
        if datos.ext_der is not None:
            self.txtExtDer.Text = "{:.0f}".format(datos.ext_der)
    ...
    def guardar_estado(self):
        ...
        # Rescatar Extensiones
        if self.txtExtIzq.Text:
            try: estado.ext_izq = float(self.txtExtIzq.Text)
            except ValueError: pass
        if self.txtExtDer.Text:
            try: estado.ext_der = float(self.txtExtDer.Text)
            except ValueError: pass
    ...
    def ExtIzqClick(self, sender, args):
        self.action = "ext_izq"
        self.guardar_estado()
        self.Close()

    def ExtDerClick(self, sender, args):
        self.action = "ext_der"
        self.guardar_estado()
        self.Close()
    ...
    def AplicarClick(self, sender, args):
        ...
        if self.txtExtIzq.Text:
            try: float(self.txtExtIzq.Text)
            except ValueError:
                forms.alert("La extensión izquierda debe ser un número válido.", title="Valor inválido")
                return
        if self.txtExtDer.Text:
            try: float(self.txtExtDer.Text)
            except ValueError:
                forms.alert("La extensión derecha debe ser un número válido.", title="Valor inválido")
                return
...
while True:
    ...
    if form.action == "ext_izq":
        try: p1, p2 = seleccionar_puntos(1)
        except OperationCanceledException: continue
        estado.ext_izq = UnitUtils.ConvertFromInternalUnits((p2 - p1).GetLength(), UnitTypeId.Millimeters)
        continue

    if form.action == "ext_der":
        try: p1, p2 = seleccionar_puntos(1)
        except OperationCanceledException: continue
        estado.ext_der = UnitUtils.ConvertFromInternalUnits((p2 - p1).GetLength(), UnitTypeId.Millimeters)
        continue
    ...
    if form.action == "aplicar":
        ...
        ext_izq = UnitUtils.ConvertToInternalUnits(estado.ext_izq if estado.ext_izq else 0, UnitTypeId.Millimeters)
        ext_der = UnitUtils.ConvertToInternalUnits(estado.ext_der if estado.ext_der else 0, UnitTypeId.Millimeters)
...
# ===================================================================================
        ...
        start = p1_3D + v_bar_3D * L1 # Extremo inicial de la barra
