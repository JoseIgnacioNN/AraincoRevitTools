...
# ===================================================================================
        ...
        offset_dist = rec_top + rebar_fi / 2 # Desplazamos el plano matemático hacia abajo para incluir recubrimiento y diámetro
        origen_desplazado = plane_origin - plane_normal * offset_dist 
        
        # Calculamos las coordenadas 3D exactas de la primera barra
        start_x = p1.X - L1 * math.sin(ang)
        start_y = p1.Y + L1 * math.cos(ang)
        start_z = get_z_on_plane(p_start_x, p_start_y, origen_desplazado, plane_normal)
        start = DB.XYZ(p_start_x, p_start_y, p_start_z) # Punto inicial de la barra
        end_x = p1.X + L2 * math.sin(ang)
        end_y = p1.Y - L2 * math.cos(ang)
        end_z = get_z_on_plane(p_end_x, p_end_y, origen_desplazado, plane_normal)
        end = DB.XYZ(p_end_x, p_end_y, p_end_z) # Punto final de la barra
        curves = [DB.Line.CreateBound(p_start, p_end)] # Las puntos que conforman la barra deben estar en una lista
        v_bar = (p_end - p_start).Normalize() # Vector director de la primera barra (en la dirección longitudinal de la barra)
        v_dir = plane_normal.CrossProduct(v_bar).Normalize() # Vector director del recorrido. Es perpendicular a la primera barra y paralelo a la pendiente de la losa

        # Asegurarnos de que el vector director apunte de p1 hacia p2
        p1_z = get_z_on_plane(p1.X, p1.Y, origen_desplazado, plane_normal)
        p2_z = get_z_on_plane(p2.X, p2.Y, origen_desplazado, plane_normal)
        v_usuario_3d = (DB.XYZ(p2.X, p2.Y, p2_z) - DB.XYZ(p1.X, p1.Y, p1_z)).Normalize()
        if v_dir.DotProduct(v_usuario_3d) < 0:
            v_dir = -v_dir

        L_recorrido_real = DB.XYZ(p2.X, p2.Y, p2_z) - DB.XYZ(p1.X, p1.Y, p1_z) # Recorrido real 
        L_recorrido = abs(L_recorrido_real.DotProduct(v_dir)) # Recorrido corregido por la distorsión que genera la losa con pendiente
...
# ===================================================================================
        ...
        # Multi-Rebar Annotation
                ...
                if tipo:
                    ...
                    tipo_opts.DimensionLineDirection = v_dir
        ...
        # Tag
                ...
                centro_primera = DB.XYZ((p_start.X + p_end.X) / 2, (p_start.Y + p_end.Y) / 2, Z_elev) # Centroide de la primera barra
                centro_barra = DB.XYZ(centro_primera.X + v_dir.X * idx_central * esp_real - offset_tag * math.sin(ang), centro_primera.Y + v_dir.Y * idx_central * esp_real + offset_tag * math.cos(ang), Z_elev) # Centroide de la barra central
