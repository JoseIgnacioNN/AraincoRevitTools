...
# ===================================================================================
        ...
        filtros_cats = [DB.ElementCategoryFilter(c) for c in categorias_busqueda]
        filtro_combinado = DB.LogicalOrFilter(List[DB.ElementFilter](filtros_cats))
                    ...
                    if isinstance(solid, DB.Solid) and solid.Faces.Size > 0: # Filtra solo las piezas sólidas (descarta líneas y puntos)
                        ...
                        for face in solid.Faces: # Itera las caras individuales de las piezas sólidas (6 caras)
                            if isinstance(face, DB.PlanarFace) and face.FaceNormal.Z > 0.5: # Busca solo caras planas que apunten hacia arriba (tolerando rampas/pendientes)
                                interseccion_fisica = face.Project(p_test) # Láser de proyección: Evalúa si hay masa física o si es un recorte/shaftinterseccion_fisica = face.Project(p_test)
        ...
        # Lógica de Puente de Unión
        if candidato_ganador:
            cat_id = candidato_ganador.Category.Id.IntegerValue
            cats_puente = [
            ...
            if cat_id in cats_puente:
                # El láser golpeó un muro, pilar o viga. Buscamos las losas unidas a él.
                unidos_ids = DB.JoinGeometryUtils.GetJoinedElements(doc, candidato_ganador)
                cats_losa = [int(DB.BuiltInCategory.OST_Floors), int(DB.BuiltInCategory.OST_StructuralFoundation)]
                losas_unidas = [doc.GetElement(id) for id in unidos_ids if doc.GetElement(id).Category.Id.IntegerValue in cats_losa]
                
                if losas_unidas:
                    losa_real = losas_unidas[0]
                    cara_losa = None
                    z_max_losa = -float('inf')
                    
                    # Extraer la pendiente real de la losa anfitriona
                    for g_obj in losa_real.get_Geometry(opt):
                        iterable = g_obj.GetInstanceGeometry() if isinstance(g_obj, DB.GeometryInstance) else [g_obj]
                        for sld in iterable:
                            if isinstance(sld, DB.Solid) and sld.Faces.Size > 0:
                                for f in sld.Faces:
                                    if isinstance(f, DB.PlanarFace) and f.FaceNormal.Z > 0.5:
                                        z_f = get_z_on_plane(mid_point.X, mid_point.Y, f.Origin, f.FaceNormal)
                                        # Tolerancia de elevación para no atrapar caras de otros pisos
                                        if z_f > z_max_losa and abs(z_f - Z_elev) < 3.28: 
                                            z_max_losa = z_f
                                            cara_losa = f
                    
                    if cara_losa:
                        slab = losa_real
                        plane_normal = cara_losa.FaceNormal
                        plane_origin = cara_losa.Origin
            else:
                # El láser golpeó directamente una losa o fundación
                slab = candidato_ganador
                plane_normal = cara_ganadora.FaceNormal
                plane_origin = cara_ganadora.Origin

        if not slab:
            t_group.RollBack()
            forms.alert("Error: Geometría inválida o la armadura se encuentra sobre un shaft.", title="Losa no válida")
            continue
