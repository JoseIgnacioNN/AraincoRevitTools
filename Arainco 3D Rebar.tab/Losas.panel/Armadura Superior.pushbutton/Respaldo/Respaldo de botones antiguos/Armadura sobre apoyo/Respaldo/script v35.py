...
class ArmaduraSuperiorForm(forms.WPFWindow):  # Funciones del formulario
    ...
    def guardar_estado(self):
        ...
        if self.txtL1.Text:
            try:
                estado.L1 = float(self.txtL1.Text)
            except ValueError:
                pass
        ...
        if self.txtL2.Text:
            try:
                estado.L2 = float(self.txtL2.Text)
            except ValueError:
                pass
...
# ===================================================================================
        ...
        # Filtro de Losas y Fundaciones
        filtro_pisos = DB.ElementCategoryFilter(DB.BuiltInCategory.OST_Floors)
        filtro_fundaciones = DB.ElementCategoryFilter(DB.BuiltInCategory.OST_StructuralFoundation)
        filtro_combinado = DB.LogicalOrFilter(filtro_pisos, filtro_fundaciones)

        mid_point = DB.XYZ((p1.X + p2.X) / 2, (p1.Y + p2.Y) / 2, p1.Z) # Punto matemático central del recorrido
        z_nivel = view.GenLevel.Elevation if view.GenLevel else p1.Z # El "Pilar Invisible": Una caja vertical muy delgada (20x20cm) en el medio del vano
        pt_min = DB.XYZ(mid_point.X - 0.328, mid_point.Y - 0.328, z_nivel - 3.28) # pt_min es la esquina inferior izquierda del pilar (1 metro hacia abajo)
        pt_max = DB.XYZ(mid_point.X + 0.328, mid_point.Y + 0.328, z_nivel + 3.28) # pt_max es la esquina superior derecha del pilar (1 metro hacia arriba)
        outline = DB.Outline(pt_min, pt_max)
        filtro_interseccion = DB.BoundingBoxIntersectsFilter(outline) # Filtro nativo real (Rápido y procesado en C++)

        # Buscamos SOLO en la vista actual losas que crucen esta caja
        candidatos = DB.FilteredElementCollector(doc, uidoc.ActiveView.Id) \
            .WherePasses(filtro_combinado) \
            .WherePasses(filtro_interseccion) \
            .WhereElementIsNotElementType() \
            .ToElements()

        if not candidatos:
            forms.alert("No se encontró ninguna losa debajo del recorrido dibujado en esta vista.", title="Losa no encontrada")
            continue

        # Plan de Rescate (Fallback): Asumimos la primera losa y su altura general
        slab = candidatos[0]
        bbox = slab.get_BoundingBox(uidoc.ActiveView)
        Z_sup = bbox.Max.Z if bbox else p1.Z

        # Intento de Súper Precisión (Solo para ajustar pendientes o superposiciones)
        opt = DB.Options()
        opt.DetailLevel = DB.ViewDetailLevel.Coarse # Pide la geometría más simple y rápida posible
        p_test = DB.XYZ(mid_point.X, mid_point.Y, pt_max.Z)
        dist_min = float('inf')
        
        # Analizar geometría de cada losa
        for candidato in candidatos:
            geom_elem = candidato.get_Geometry(opt)
            if not geom_elem: continue
            
            cara_encontrada = False # Usamos una "bandera" para saber si ya terminamos con esta losa
            for geom_obj in geom_elem:
                if cara_encontrada: break # Freno de mano nivel objeto
                iterable_geom = geom_obj.GetInstanceGeometry() if isinstance(geom_obj, DB.GeometryInstance) else [geom_obj]
                for solid in iterable_geom:
                    if cara_encontrada: break # Freno de mano nivel sólido

                    if isinstance(solid, DB.Solid) and solid.Faces.Size > 0:
                        for face in solid.Faces:
                            if isinstance(face, DB.PlanarFace) and face.FaceNormal.Z > 0.5: # Buscar caras superiores (Que apunten hacia arriba, tolerando losas con pendiente)
                                proj = face.Project(p_test) # Revisa si la proyección es válida (el punto cae dentro de la silueta de la cara). Si falla, el código simplemente usará la losa y la altura del "Plan de Rescate"
                                if proj:
                                    dist = abs(proj.XYZPoint.Z - z_nivel)
                                    if dist < dist_min: # Nos quedamos con la cara más cercana al nivel actual
                                        dist_min = dist
                                        slab = candidato
                                        Z_sup = proj.XYZPoint.Z # Actualizamos con la elevación matemática exacta
        ...
        try:
            ...
            forms.alert("La losa detectada no tiene parámetros de recubrimiento estructural válidos.", title="Error de Parámetros")
            continue
