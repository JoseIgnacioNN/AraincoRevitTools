...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def __init__(self, xaml_file, datos):
        ...
        if datos.ext_val is not None:
            self.txtExt.Text = "{:.0f}".format(datos.ext_val)
        if datos.ext_loc == "Derecha":
            self.chkExtDer.IsChecked = True
        else:
            self.chkExtIzq.IsChecked = True
    ...
    def guardar_estado(self):
        ...
        if self.txtExt.Text:
            try: estado.ext_val = float(self.txtExt.Text)
            except ValueError: pass
        if self.chkExtDer.IsChecked:
            estado.ext_loc = "Derecha"
        else:
            estado.ext_loc = "Izquierda"
    ...
    def ChkIzq_Click(self, sender, args):
        self.chkExtIzq.IsChecked = True
        self.chkExtDer.IsChecked = False

    def ChkDer_Click(self, sender, args):
        self.chkExtIzq.IsChecked = False
        self.chkExtDer.IsChecked = True
    ...
    def AplicarClick(self, sender, args):
        ...
        if self.txtExt.Text:
            try: float(self.txtExt.Text)
            except ValueError:
                forms.alert("La extensión debe ser un número válido.", title="Valor inválido")
                return
...
while True:
    ...
    if form.action == "ext":
        ...
        estado.ext_val = UnitUtils.ConvertFromInternalUnits((p2 - p1).GetLength(), UnitTypeId.Millimeters)
        continue
    ...
    if form.action == "aplicar":
        ...
        ext_val = UnitUtils.ConvertToInternalUnits(estado.ext_val if estado.ext_val else 0, UnitTypeId.Millimeters)
        ext_loc = estado.ext_loc
