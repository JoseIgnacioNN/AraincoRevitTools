...
while True:
    ...
    if form.action == "aplicar":
        ...
        # ===================================================================================
        # NUEVO: AUTODETECCIÓN DE LA LOSA O FUNDACIÓN
        # ===================================================================================
        ...
        # 2. El "Pilar Invisible": Una caja vertical muy delgada (20x20cm) en el medio del vano
        z_nivel = view.GenLevel.Elevation if view.GenLevel else p1.Z
        
        # pt_min es la esquina inferior izquierda del pilar (1 metro hacia abajo)
        pt_min = DB.XYZ(mid_point.X - 0.328, mid_point.Y - 0.328, z_nivel - 3.28) 
        # pt_max es la esquina superior derecha del pilar (1 metro hacia arriba)
        pt_max = DB.XYZ(mid_point.X + 0.328, mid_point.Y + 0.328, z_nivel + 3.28) 
        
        outline = DB.Outline(pt_min, pt_max)
        
        # 3. Filtro nativo real (Rápido y procesado en C++)
        filtro_interseccion = DB.BoundingBoxIntersectsFilter(outline)
        ...
        # 4. Plan de Rescate (Fallback): Asumimos la primera losa y su altura general
        slab = candidatos[0]
        bbox = slab.get_BoundingBox(uidoc.ActiveView)
        Z_sup = bbox.Max.Z if bbox else p1.Z

        # 5. Intento de Súper Precisión (Solo para ajustar pendientes o superposiciones)
        opt = DB.Options()
        ...
        # ¡OJO! Ya no hay alerta de "Error de Geometría". 
        # Si la proyección falla, el código simplemente usará la losa y la altura del "Plan de Rescate".

        # Extraer parámetros de la losa encontrada
        try:
