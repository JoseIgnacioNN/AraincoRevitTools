...
# ===================================================================================
        ...
        # Desplazamos el plano matemático hacia abajo para incluir recubrimiento y diámetro
        offset_dist = rec_top + rebar_fi / 2
        origen_desplazado = plane_origin - plane_normal * offset_dist 

        ang = math.atan2(p2.Y - p1.Y, p2.X - p1.X)

        # 1. Calculamos las coordenadas 3D exactas de la PRIMERA barra (en p1)
        p_start_x = p1.X - L1 * math.sin(ang)
        p_start_y = p1.Y + L1 * math.cos(ang)
        p_start_z = get_z_on_plane(p_start_x, p_start_y, origen_desplazado, plane_normal)
        p_start = DB.XYZ(p_start_x, p_start_y, p_start_z)

        p_end_x = p1.X + L2 * math.sin(ang)
        p_end_y = p1.Y - L2 * math.cos(ang)
        p_end_z = get_z_on_plane(p_end_x, p_end_y, origen_desplazado, plane_normal)
        p_end = DB.XYZ(p_end_x, p_end_y, p_end_z)

        curves = [DB.Line.CreateBound(p_start, p_end)] 

        # 2. LA MAGIA MATEMÁTICA: Vectores 3D y Producto Cruz
        # Obtenemos el vector direccional exacto de la barra en el espacio 3D
        v_bar = (p_end - p_start).Normalize() 
        
        # Calculamos el vector de distribución usando Producto Cruz
        # Este vector es perpendicular a la barra y paralelo a la pendiente de la losa
        v_dir = plane_normal.CrossProduct(v_bar).Normalize()

        # 3. Orientación y Longitud
        # Asegurarnos de que el vector de distribución apunte hacia p2 (hacia adelante)
        ...
        # Calculamos el largo real del recorrido proyectándolo sobre nuestro vector perfecto
        v_recorrido_real = DB.XYZ(p2.X, p2.Y, p2_z) - DB.XYZ(p1.X, p1.Y, p1_z)
        L_recorrido = abs(v_recorrido_real.DotProduct(v_dir))
