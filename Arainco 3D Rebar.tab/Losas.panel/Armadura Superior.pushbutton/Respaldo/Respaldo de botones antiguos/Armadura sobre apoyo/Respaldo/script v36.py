...
# ===================================================================================
        ...
        pt_min = DB.XYZ(mid_point.X - 0.3281, mid_point.Y - 0.3281, Z_elev - 3.281) # Esquina inferior izquierda del pilar
        pt_max = DB.XYZ(mid_point.X + 0.3281, mid_point.Y + 0.3281, Z_elev + 3.281) # Esquina superior derecha del pilar
        filtro_pilar = DB.BoundingBoxIntersectsFilter(DB.Outline(pt_min, pt_max)) # Pilar rectangular 20x20x200 cm, que que se utiliza para intersectar con la losa
        ...
        if not candidatos:
            forms.alert("No se encontró ninguna losa visible en un rango de ±1 metro, respecto al nivel de la vista actual.", title="Losa no encontrada")
            continue
                    ...
                    if isinstance(solid, DB.Solid) and solid.Faces.Size > 0: # Filtra solo las piezas sólidas (descarta líneas y puntos)
                        for face in solid.Faces: # Itera las caras individuales de las piezas sólidas (6 caras)
                            if isinstance(face, DB.PlanarFace) and face.FaceNormal.Z > 0.5: # Busca solo caras planas que apunten hacia arriba (tolerando rampas/pendientes)
                                proj = face.Project(p_test) # Dispara un rayo vertical desde p_test hacia la cara
                                if proj: # Revisa si la proyección es válida. Si falla, el código simplemente usará la elevación del "Plan de Rescate"
                                    dist = abs(proj.XYZPoint.Z - Z_elev) # Distancia entre la cara de la losa candidata y la elevación del nivel actual
                                    if dist < dist_min:
                                        dist_min = dist
                                        slab = candidato
                                        Z_sup = proj.XYZPoint.Z # Elevación precisa de la losa, en el punto medio del recorrido
