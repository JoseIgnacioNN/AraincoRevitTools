...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def AplicarClick(self, sender, args):
        ...
        if not self.txtL.Text:
            forms.alert("Debes seleccionar un vano para definir el largo de la barra.", title="Falta información")
            return
...
while True:
    ...
    if form.action == "vano":
        ...
        estado.L = estado.largo
        centro_vano = (p1+p2)/2
        v_vano_2D = (p2-p1).Normalize()
...
# ===================================================================================
        ...
        if abs(v_vano_2D.DotProduct(v_recorrido_2D)) > 0.001: # Si el valor absoluto es mayor a la tolerancia, NO son perpendiculares
            t_group.RollBack()
        ...
        centro_z = get_z_on_plane(centro_vano.X, centro_vano.Y, origen_desplazado, plane_normal)
        centro_3D = DB.XYZ(centro_vano.X, centro_vano.Y, centro_z)
        desplazamiento = (p1_3D - centro_3D).DotProduct(v_recorrido_3D)
        centro_barra = centro_3D + (v_recorrido_3D * desplazamiento)
