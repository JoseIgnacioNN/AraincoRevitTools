...
from System.Windows import SystemParameters, Window, WindowStartupLocation, Thickness, WindowState
from System.Windows.Controls import Image as WpfImage, Grid as WpfGrid
...
class InfoWindow(forms.WPFWindow):
    def __init__(self, xaml_file):
        ...
        if os.path.exists(ruta_img):
            ...
            forms.alert("No se encontró la imagen 'Esquema Losa.png'.", title="Imagen no encontrada")
...
class Formulario(forms.WPFWindow):  # Funciones del formulario
    ...
    def InfoClick(self, sender, args):
        xaml_info = os.path.join(os.path.dirname(__file__), "Esquema Losa.xaml")
        if os.path.exists(xaml_info):
            win = InfoWindow(xaml_info)
            win.ShowDialog()
        else:
            forms.alert("No se encontró el archivo 'Esquema Losa.xaml'.", title="Archivo no encontrado")
...
# ===================================================================================
        ...
        p1_3D = p1_3D + v_recorrido_3D * offset_lateral # Punto inicial del recorrido, reducido por el recubrimiento lateral del borde de la losa
