...
while True:
    ...
    if form.action == "aplicar":        
        ...
        # Transaction Group
        t_group = DB.TransactionGroup(doc, "ARAINCO - Armadura Superior (Malla en 2 direcciones)")
        t_group.Start()
...
# ===================================================================================
        ...
        sets_bar = [
        {"set": "vano 1", "vano": (p1_vano1, p2_vano1), "recorrido": (p1_vano2, p2_vano2), "rebar_selected": rebar_types[estado.sel_index], "rebar_opos": rebar_types[estado.sel_index2], "esp": UnitUtils.ConvertToInternalUnits(float(estado.esp_text), UnitTypeId.Millimeters), "rec": rec_vano1, "t1_diff": t1_diff_vano1, "ubicacion": ubic_vano1},
        {"set": "vano 2", "vano": (p1_vano2, p2_vano2), "recorrido": (p1_vano1, p2_vano1), "rebar_selected": rebar_types[estado.sel_index2], "rebar_opos": rebar_types[estado.sel_index], "esp": UnitUtils.ConvertToInternalUnits(float(estado.esp_text2), UnitTypeId.Millimeters), "rec": rec_vano2, "t1_diff": t1_diff_vano2, "ubicacion": ubic_vano2}]
        ...
        for bar in sets_bar:
            ...
            pt2_2 = pt1_2 + v_bar_3D * t2 # Tramo 2 del gancho 2

            curves = [DB.Line.CreateBound(pt2_1, pt1_1), 
                    DB.Line.CreateBound(pt1_1, start), 
                    DB.Line.CreateBound(start, end), 
                    DB.Line.CreateBound(end, pt1_2), 
                    DB.Line.CreateBound(pt1_2, pt2_2)]
...
# ===================================================================================
        ...
        # Offset para Multi-Rebar Annotation y Tag
                if bar["set"] == "vano 1":
                    ...
                    offset_tag = UnitUtils.ConvertToInternalUnits(-75, UnitTypeId.Centimeters)
                else:
                    offset_mra = UnitUtils.ConvertToInternalUnits(75, UnitTypeId.Centimeters) - L_barra/2
                    offset_tag = UnitUtils.ConvertToInternalUnits(75, UnitTypeId.Centimeters)
