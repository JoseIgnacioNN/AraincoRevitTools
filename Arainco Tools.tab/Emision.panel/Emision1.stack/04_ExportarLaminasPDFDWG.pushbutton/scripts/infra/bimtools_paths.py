...
``icon.png`` (icono del botón en la cinta): no se usa en la cabecera del formulario
``Exportar Láminas``; ahí solo se muestran ``empresa_logo.png``, ``logo_empresa.png`` o
``logo.png`` (colóquelos en esta carpeta del pushbutton para portabilidad entre extensiones).
El ``icon.png`` puede seguir usándose como icono de la ventana en la barra de tareas.

``raíz_extensión`` se detecta así:
  - Layout original: este archivo en ``.../Ext.extension/scripts/bimtools_paths.py`` → padre del directorio ``scripts``.
  - Copia en un ``.pushbutton``: se suben directorios hasta uno cuyo nombre termine en ``.extension``
    (convención pyRevit).
"""
...
def _bitmap_image_from_file(path):
    """
    Carga un PNG/JPEG vía FileStream (patrón fiable en IronPython / pyRevit).
    """
    try:
        import clr
...
def default_extension_root():
    """
    Raíz de la extensión pyRevit (carpeta ``*.extension``).

    Soporta este archivo en ``.../Ext.extension/scripts/`` o dentro de un ``.pushbutton``.
    """
    d = os.path.dirname(os.path.abspath(__file__))
    for _ in range(16):
        base = os.path.basename(d)
        try:
            if base.lower().endswith(u".extension"):
                return d
        except Exception:
            pass
        ...
        if parent == d:
            break
        d = parent
    # Fallback layout histórico: .../extension/scripts/bimtools_paths.py
...
def get_header_logo_paths(extension_root=None):
    """
    Rutas solo para imagen corporativa en cabecera WPF (``empresa_logo.png``,
    ``logo_empresa.png``, ``logo.png`` en pushbutton, ``assets/``, ``branding/`` y
    fallbacks BIMTools). **No** incluye ``icon.png`` de la cinta.
    """
    if extension_root is None:
        extension_root = default_extension_root()
    else:
...
def load_wpf_header_logo_bitmap_image():
    """Primer logo corporativo disponible según :func:`get_header_logo_paths` (sin ``icon.png``)."""
    for path in get_header_logo_paths():
        bmp = _bitmap_image_from_file(path)
        if bmp is not None:
            return bmp
    return None


def load_pushbutton_icon_bitmap_image():
    """Solo ``icon.png`` en la carpeta del pushbutton (icono de ventana / barra de tareas)."""
    if not _pushbutton_dir:
        return None
    return _bitmap_image_from_file(os.path.join(_pushbutton_dir, "icon.png"))
...
def load_logo_bitmap_image():
    ...
    Incluye ``icon.png`` como último recurso (comportamiento histórico para otras vistas).
    """
